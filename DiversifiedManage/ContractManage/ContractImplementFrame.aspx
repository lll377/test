<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="ContractImplementFrame.aspx.cs" Inherits="M_Main.DiversifiedManage.ContractManage.ContractImplementFrame" %>
...
<body>
    <form id="frmForm" runat="server">
        ...
        <div id="tab" class="easyui-tabs" data-options="fit:true,tabWidth:'100'">
                ...
                <fieldset class="frame-fieldset">
                    ...
                    <table class="bootstrap_table baseinfo" border="0" cellspacing="0" cellpadding="0">
                        ...
                        <tr>
                            ...
                            <td class="text" colspan="5">
                                <input id="DutyDepNameAndUserName" name="DutyDepNameAndUserName" class="easyui-textbox" disabled="disabled" data-options="width:'52%'" runat="server" />
            ...
            <div id="tab1" data-issel="false" title="合同执行信息" style="overflow: hidden;">
    ...
    <script type="text/javascript">
        ...
        var ImplementColumn = [[
            { field: 'CommName', title: '项目名称', width: 140, align: 'center' },
            {
                field: 'ContractSign', title: '合同编号', width: 140, align: 'center', formatter: function (value, row, index) {
                    var str = "<a class=\"HrefStyle\"  href='#' onclick=\"ImplementDetail('edit','" + row.ID + "');\">" + row.ContractSign + "</a>";
                    return str;
                }
            },
            {
        ...
        var ImplementToolbar = [
            {
                text: '新增',
                iconCls: 'icon-add',
                handler: function () {
                    //判断 判断合同是否是已终止 已终止
                    $.tool.DataPostJson('DiversifiedManage', 'ContractImplementValidate', "ContractID=" + $('#ContractID').val(),
                        function Init() {
                        },
            ...
            }, '-',
            {
                ...
                handler: function () {
                    ...
                    if (row == null) {
                        $.messager.alert('温馨提示', '请选择需要删除的合同执行数据!');
                        return;
                    }
                    if (row.AuditStatus != "未启动" || row.AuditStatus == "已驳回") {
                        $.messager.alert('温馨提示', '当前合同执行不允许删除!');
                        return;
                    }
                    $.messager.confirm('温馨提示', '确定删除选择的合同执行数据?', function (r) {
                        if (r) {
                            $.tool.DataPostJson('DiversifiedManage', 'DelContractImplement', "idList=" + row.ID,
                                function Init() {
                                },
                                function callback(data) {
                                    if (data.result) {
        ...
        function ImplementDetail(OpType, ID) {
            var param = { "OpType": OpType, "Id": ID, "ContractID": $('#ContractID').val() };
            LayerDialog.OpenWin("1000", '600', '../DiversifiedManage/ContractManage/ContractImplementDetail.aspx?' + $.param(param), "合同执行详情", false, function callback(_Data) {
                if (_Data != "") {
                    LoadImplementGrid();
                }
            });
        ...
        function LoadImplementGrid() {
            $("#TableContainerImplement").datagrid({
                ...
                onBeforeLoad: function (param) {
                    ...
                    param.Command = "GetContractImplementDetailList";
        ...
        function LoadFeesGrid() {
            ...
            $("#TableContainerContractFees").datagrid({
                ...
                onBeforeLoad: function (param) {
                    param.Method = "DataPost";
                    param.Class = "DiversifiedManage";
                    param.Command = "ContractFeesList_ImplementAndRoyalty";
