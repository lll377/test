<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="CheckRoomSet.aspx.cs" Inherits="M_Main.HouseInspect.CheckRoomSet" %>
    ...
    <form id="frmFrom" runat="server">
            ...
            <table style="width: 100%; font-size: 12px; font-family: 微软雅黑;">
                <tr>
                    ...
                    <td class="TdContent">
                      <%--  <select id="RegionSNum" name="RegionSNum" class="easyui-combobox" data-options=" multiple: true " runat="server" style="width: 153px; height: 20px">
                        ...
                        <input id="RegionSNum" name="RegionSNum" class="easyui-combobox" data-options="editable:false,tipPosition:'bottom',panelHeight:'auto',
                    ...
                    <td class="TdContent">
                       <%-- <select id="BuildSNum" runat="server" name="BuildSNum" style="width: 153px; height: 20px" class="easyui-combobox" data-options=" multiple: true ">
                         ...
                         <input id="BuildSNum" name="BuildSNum" class="easyui-combobox" style="height: 20px" data-options="editable:false,tipPosition:'bottom',
                                    valueField:'id',textField:'text',multiple:true" />
                       <%-- <input type="hidden" id="BuildID" name="BuildID" runat="server"/>
                    <input type="text" class="easyui-searchbox" style="height: 20px; width: 156px;" id="BuildName" name="BuildName" data-options="editable:false" searcher="SelBuilding" runat="server" />--%>
                ...
                <tr>
                    <td colspan="8" align="center"><a href="#" class="easyui-linkbutton" data-options="iconCls:'icon-search'" onclick="LoadList();">确定筛选</a>
                        <a href="#" class="easyui-linkbutton" data-options="iconCls:'icon-clear'" onclick="$('#frmFrom').form('clear');">清空</a>
                    </td>
                </tr>
            </table>
        </div>
        ...
        <script language="javascript" type="text/javascript">
            ...
            function GetParam() {
                ...
                //}
                if ($("#RegionSNum").combobox("getValues").length > 0) {
                    str += '&RegionSNum=' + $("#RegionSNum").combobox("getValues")
            ...
            var column = [[
                ...
                {
                    field: 'ZRUnitName', title: '查验房间', width: 20, align: 'left', sortable: true, formatter: function (value, row, index) {
                        var str = "<a sclass=\"HrefStyle\" onclick=\"CheckRoomSetSee('" + row.CommID + "','" + row.RoomID+"');\" href=\"#\">查看</a>";
                        return str;
                    }
                },
                {
            ...
            function LoadList() {
                ...
                $("#TableContainerPoint").datagrid({
                    ...
                    //    param = $.JQForm.GetParam("CsHouseInspect", "CsHouseInspect", "TableContainerPoint", GetParam());
                    //},
            ...
            //查看对象,责任/整改单位详情
            function CheckRoomSetSee(CommID, RoomID) {
                $('.easyui-dialog').parent().appendTo($("form:first"));
                SetCache();
                HDialog.Open('800', '600', '../HouseInspect/CheckRoomSetSee.aspx?CommID=' + CommID + "&RoomID=" + RoomID, '查验房间', false, function callback(_Data) {
                    if (_Data != '') {
                        // LoadList();
                    }
                });
            ...
            function SetCache() {
                ...
                //}
                if ($("#RegionSNum").combobox("getValues").length > 0) {
                    Cache.SetData("RegionSNum", $("#RegionSNum").combobox("getValues"));

                } else {
                    Cache.SetData("RegionSNum", "");
                }
            ...
            function SetUnit() {
                ...
                //全选时使用的筛选条件,放到缓存中
                SetCache();
                HDialog.Open('1000', '600', '../HouseInspect/CheckRoomSetFrame.aspx?RoomIDs=' + $("#RoomIDs").val() + "&CommID=" + $("#CommId").val() + "&CheckAll=" + $("#CheckAll").val(), '设置', false, function callback(_Data) {
                    if (_Data != '') {
                        // LoadList();
                    }
                });
            ...
            //加载组图区域
            function GetRegionIDList(HiCommID) {
                var varObjects;
                $.tool.DataPostAsync('CsHouseInspect', 'GetRegionIDList', 'CommID=' + HiCommID,
                    function Init() {
                    },
                    function callback(_Data) {
                        varObjects = eval("(" + _Data.replace(/\\/g, "/") + ")"); //转换为json对象
                        ...
                        console.log(varObjects)
                        $("#RegionSNum").find("option:gt(0)").remove();
                        $("#RegionSNum").combobox({
                            width: 156,
