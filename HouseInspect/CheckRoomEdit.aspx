<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="CheckRoomEdit.aspx.cs" Inherits="M_Main.HouseInspect.CheckRoomEdit" %>
    ...
    <script>
        function Load() {
            ...
            if (GetQueryString('OpType') == 'edit') {
                $.tool.DataGet('CsHouseInspect', 'GetCheckRoomDefinitionList', 'ID=' + $("#Id").val() + '&page=1&rows=1',
                    function Init() {
                    },
                    function callback(_Data) {
                        varObjects = eval("(" + _Data.replace(/\\/g, "/") + ")"); //转换为json对象
                        $("#Sort").val(varObjects.rows[0].Sort);
                        $("#HouseName").val(varObjects.rows[0].HouseName);
                        $("#RoomType").val(varObjects.rows[0].RoomType);
        ...
        function Save() {
            $.tool.DataPostChk('frmFormStandard', 'CsHouseInspect', 'SaveCheckRoomDefinition', $('#frmFormStandard').serialize(),
                function Init() {
                },
                function callback(_Data) {
                    HDialog.Close("true");
