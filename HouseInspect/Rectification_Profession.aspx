<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Rectification_Profession.aspx.cs" Inherits="M_Main.HouseInspect.Rectification_Profession" %>
    ...
    <form id="FrmForm" runat="server">
            ...
            <table style="width: 100%; padding-top: 5px">
                <tr>
                  <%--  <td class="TdTitle">项目名称
                    ...
                    <td class="TdContent">
                        <input type="text" class="easyui-validatebox" id="CommName" name="CommName" runat="server" style="width: 95%; border: 1px solid #cccccc;" onclick="SelComm();" />
        ...
        <script language="javascript" type="text/javascript">
            ...
            var column = [[
                ...
                { field: 'BatchName', title: '查验批次', width: 100, align: 'left', sortable: false },
                { field: 'Profession', title: '所属专业', width: 200, align: 'left', sortable: false, hidden: true },
                { field: 'ProfessionName', title: '所属专业', width: 100, align: 'left', sortable: false },
                {
                    field: 'HouseNum', title: '全部房屋/公区数量', width: 120, align: 'center', halign: 'left', sortable: false,
                    formatter: function (value, row, index) {
                        var str = "<a class=\"HrefStyle\"  href=\"#\" onclick=\"Rectification_HousesInfo('" + row.CommId + "','" + row.Batch + "','" + row.Profession + "','All')\">" + row.HouseNum + "</a>";
                        return str;
                    }
                },
                {
                    field: 'CompleteHouseNum', title: '完成房屋/公区数量', width: 120, align: 'center', halign: 'left', sortable: false,
                    formatter: function (value, row, index) {
                        var str = "<a class=\"HrefStyle\"  href=\"#\" onclick=\"Rectification_HousesInfo('" + row.CommId + "','" + row.Batch + "','" + row.Profession + "','Complete')\">" + row.CompleteHouseNum + "</a>";
                        return str;
                    }
                },
                {
                    field: 'QualifiedHouseNum', title: '合格房屋/公区数量', width: 120, align: 'center', halign: 'left', sortable: false,
                    formatter: function (value, row, index) {
                        var str = "<a class=\"HrefStyle\"  href=\"#\" onclick=\"Rectification_HousesInfo('" + row.CommId + "','" + row.Batch + "','" + row.Profession + "','Qualified')\">" + row.QualifiedHouseNum + "</a>";
                        return str;
                    }
                },
                {
                    field: 'UnqualifiedHouseNum', title: '问题房屋/公区数量', width: 120, align: 'center', halign: 'left', sortable: false,
                    formatter: function (value, row, index) {
                        var str = "<a class=\"HrefStyle\"  href=\"#\" onclick=\"Rectification_HousesInfo('" + row.CommId + "','" + row.Batch + "','" + row.Profession + "','Unqualified')\">" + row.UnqualifiedHouseNum + "</a>";
                        return str;
                    }
                },
                {
                    field: 'CheckedHouseNum', title: '验收房屋/公区数量', width: 120, align: 'center', halign: 'left', sortable: false,
                    formatter: function (value, row, index) {
                        var str = "<a class=\"HrefStyle\"  href=\"#\" onclick=\"Rectification_HousesInfo('" + row.CommId + "','" + row.Batch + "','" + row.Profession + "','Checked')\">" + row.CheckedHouseNum + "</a>";
                        return str;
                    }
                }
            ]];
            ...
            //查看整改详情
            function Rectification_HousesInfo(CommID, Batch, Profession, CheckType) {
                $('.easyui-dialog').parent().appendTo($("form:first"));
                HDialog.Open('1000', '570', '../HouseInspect/Rectification_HousesInfo.aspx?CommID=' + CommID + "&Batch=" + Batch + '&Profession=' + Profession + '&CheckType=' + CheckType, '房屋/公区详情', false, function callback(_Data) {
                });
            }

            //导出
            function ExcelPrint() {
                var url = '../HouseInspect/Excel.aspx?ExcelName=按所属专业查看' + '&random=' + Math.random() + GetParamet();
                window.open(url);
            }
            ...
            function LoadList() {
                $("#TableContainer").datagrid({
                    ...
                    onBeforeLoad: function (param) {
                        param = $.JQForm.GetParam("CsHouseInspect", "Rectification_Profession", "TableContainer", param);
                    }
                });
