<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="CheckRoom.aspx.cs" Inherits="M_Main.HouseInspect.CheckRoom" %>
...
<body style="margin: 0px; padding: 0px; overflow: hidden;">
        ...
        <div id="SearchDlg" class="easyui-dialog" title="筛选条件" data-options="iconCls:'icon-search',closed: true,modal:true" style="width: 600px; height: 130px; padding: 10px;">
            <table style="width: 100%; font-size: 12px; font-family: 微软雅黑;">
                <tr>
                    ...
                    <td class="TdContent">
                        <select id="RoomType" name="RoomType" data-options="required:true" isdctype='true' dctype="房间类型" class="easyui-validatebox" style="border: 1px #cccccc solid; width: 204px;height:22px">
                        </select>
                    </td>
                    <td class="TdTitle">房间名称:</td>
                    <td class="TdContent">
                        <input id="HouseName" name="HouseName" type="text" class="easyui-validatebox" style="border: 1px #cccccc solid; width: 200px;height:20px" data-options="required:true" />
                    </td>
                </tr>
                <tr>
                    <td></td>
    ...
    <script>
        ...
        var column = [[
            ...
            {
                field: 'HouseName', title: '房间名称', width: 70, align: 'left', sortable: true, formatter: function (value, row, index) {
                    var str = "<a class=\"HrefStyle\" href=\"#\" onclick=\"Detail('" + row.Id + "','edit');\">" + row.HouseName + "</a>";
                    return str;
                }
            },
        ...
        function DelStandard(row) {
            $.messager.confirm('确定', '是否删除该查验房屋', function (r) {
                if (r) {
                    $.tool.DataGet('CsHouseInspect', 'DelCheckRoomDefinition', 'ID=' + row.Id,
                        function Init() {
                        },
        ...
        function Detail(ID, OpType) {
            ...
            HDialog.Open('670', '250', '../HouseInspect/CheckRoomEdit.aspx?CommID=' + $("#CommId").val() + '&Id=' + ID + '&OpType=' + OpType, '查验房屋编辑', false, function callback(_Data) {
                if (_Data != '') {
                    LoadList();
                }
            });
