<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Rectification_DutyUnit.aspx.cs" Inherits="M_Main.HouseInspect.Rectification_DutyUnit" %>
    ...
    <form id="FrmForm" runat="server">
            ...
            <table style="width: 100%; padding-top: 5px">
                <tr>
                  <%--  <td class="TdTitle">项目名称
                    ...
                    <td class="TdContent">
                        <input type="text" class="easyui-validatebox" id="UnitName" name="UnitName" runat="server" onclick="SelResponsibleUnit();" style="width: 95%; border: 1px solid #cccccc;" />
        ...
        <script language="javascript" type="text/javascript">
            ...
            var column = [[
                ...
                { field: 'DutyUnitId', title: '责任单位id', width: 200, align: 'left', sortable: false, hidden: true },
                { field: 'UnitName', title: '责任单位', width: 120, align: 'left', sortable: false },
                {
                    field: 'UnqualifiedHouseNum', title: '问题房屋/公区数量', width: 120, align: 'center', halign :'left', sortable: false,
                    formatter: function (value, row, index) {
                        var str = "<a class=\"HrefStyle\"  href=\"#\" onclick=\"Rectification_HousesInfo('" + row.CommId + "','" + row.Batch + "','" + row.DutyUnitId + "','Unqualified')\">" + row.UnqualifiedHouseNum + "</a>";
                        return str;
                    }
                },
                {
                    ...
                    formatter: function (value, row, index) {
                        var str = "<a class=\"HrefStyle\"  href=\"#\" onclick=\"Rectification_HousesInfo('" + row.CommId + "','" + row.Batch + "','" + row.DutyUnitId + "','Checked')\">" + row.CheckedHouseNum + "</a>";
                        return str;
                    }
                }
            ]];
            ...
            //查看整改详情
            function Rectification_HousesInfo(CommID, Batch, DutyUnit, CheckType) {
                $('.easyui-dialog').parent().appendTo($("form:first"));
                HDialog.Open('1000', '570', '../HouseInspect/Rectification_HousesInfo.aspx?CommID=' + CommID + "&Batch=" + Batch + "&DutyUnit=" + DutyUnit + '&CheckType=' + CheckType, '房屋/公区详情', false, function callback(_Data) {
                });
            }

            //导出
            function ExcelPrint() {
                var url = '../HouseInspect/Excel.aspx?ExcelName=按责任单位查看' + '&random=' + Math.random() + GetParamet();
                window.open(url);
            }
            ...
            function LoadList() {
                $("#TableContainer").datagrid({
                    ...
                    onBeforeLoad: function (param) {
                        param = $.JQForm.GetParam("CsHouseInspect", "Rectification_DutyUnit", "TableContainer", param);
                    }
                });
