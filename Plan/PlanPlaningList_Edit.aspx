<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="PlanPlaningList_Edit.aspx.cs" Inherits="M_Main.Plan.PlanPlaningList_Edit" %>
    ...
    <form id="frmForm" method="post">
        <table class="DialogTable" style="width: 100%; padding-top: 5px; line-height: 25px;">
            ...
            <tr>
                ...
                <td class="TdContentSearch">
                    <input type="text" class="easyui-validatebox" id="DutyUserName" style="width: 80%;" name="DutyUserName" onclick="SelDutyUser()" runat="server" />
            ...
            <tr>
                ...
                <td class="TdContentSearch">
                    <input type="text" class="easyui-validatebox" id="AssistManName" style="width: 80%;" name="AssistManName" onclick="SelAssistMan()" />
                    <input type="hidden" id="AssistMan" name="AssistMan" />
                </td>
                <td class="TdTitle">考评人：</td>
                <td class="TdContentSearch">
                    <input type="text" class="easyui-validatebox" id="EvaluationUserName" style="width: 80%;" name="EvaluationUserName" onclick="SelEvaluationUser()" />
                    <input type="hidden" id="EvaluationUser" name="EvaluationUser" />
                </td>
            </tr>
            <%--   <tr>
                <td class="TdTitle">计划备注：</td>
                <td colspan="3" class="TdContent">
                    <textarea style="width: 92%; height: 30px;" runat="server" id="PlanRemark" name="PlanRemark"></textarea>
                </td>
            </tr>--%>
            <tr>
        ...
        <script type="text/javascript">
            ...
            function SelDutyUser() {
                ...
                localStorage.setItem("SelUserCode", $("#DutyUser").val());
                localStorage.setItem("SelUserName", $("#DutyUserName").val());
                ...
                HDialog.Open(w, h, '../Plan/SelUser.aspx?IsCheck=0', "选择责任人", false, function callback(_Data) {
                    ...
                    if (_Data > '') {
                        $("#DutyUser").val(_Data.split('|')[0]);
                        $("#DutyUserName").val(_Data.split('|')[1]);
                    }
                });
            }

            //选择协助人
            function SelAssistMan() {
                var w = top.$(window).width();
                var h = top.$(window).height();
                localStorage.setItem("SelUserCode", $("#AssistMan").val());
                localStorage.setItem("SelUserName", $("#AssistManName").val());

                HDialog.Open(w, h, '../Plan/SelUser.aspx?IsCheck=0', "选择协助人", false, function callback(_Data) {
                    localStorage.removeItem('SelUserCode');
                    localStorage.removeItem('SelUserName');
                    if (_Data > '') {
                        $("#AssistMan").val(_Data.split('|')[0]);
                        $("#AssistManName").val(_Data.split('|')[1]);
                    }
                });
            }
            ...
            function SelEvaluationUser() {
                var w = top.$(window).width();
                var h = top.$(window).height();
                localStorage.setItem("SelUserCode", $("#EvaluationUser").val());
                localStorage.setItem("SelUserName", $("#EvaluationUserName").val());

                HDialog.Open(w, h, '../Plan/SelUser.aspx?IsCheck=0&IsNoControl=0', "选择考评人", false, function callback(_Data) {
                    localStorage.removeItem('SelUserCode');
                    localStorage.removeItem('SelUserName');
                    if (_Data > '') {
                        $("#EvaluationUser").val(_Data.split('|')[0]);
                        $("#EvaluationUserName").val(_Data.split('|')[1]);
                    }
                });
            }
            ...
            function LoadData() {
                ...
                $('#UploadFrame').attr("src", "../Plan/PlanUploadFile.aspx?DetailId=" + $('#DetailId').val() + "&SortName=计划编制");
                if ($("#OpType").val() == 'edit') {
                    //initSelect();
                    //加载直接传过来的对象
                    ...
                    if (Rows != undefined && Rows != "") {
                        ...
                        if (Rows.RemindDate != "") {
                            $("input[type = 'checkbox']").each(function () {
                                if (Rows.RemindDate.includes($(this).val())) {
                ...
                if ($("#PlanCycle").val() == '月度计划') //10， 计划编制时若计划类型选择的年，新增计划时可以选择月份，若选择的是月，则只能默认计划单的月份，不能修改
                {
                ...
                if ($("#frmForm").form('validate')) {
                    ...
                    $("input[type='checkbox']").each(function () {
                        if ($(this).prop("checked")) {
                            RemindDateStr += $(this).val() + ",";
                        }
                    });
                    if (RemindDateStr.length > 0) {
                        RemindDateStr = RemindDateStr.substr(0, RemindDateStr.length - 1);
                    }
                    for (var i = 0; i < PlanMonthS.length; i++) {
                        var DetailIDTemp = "";
                        var PlanStartTime = "";
                        ...
                        var ReturnJsonObj = {
                            ...
                            DutyUser: $('#DutyUser').val(),//责任人
                            EvaluationUser: $('#EvaluationUser').val(),//考评人
                            RemindDate: RemindDateStr,//提醒时间
                            PlanRemark: '',
