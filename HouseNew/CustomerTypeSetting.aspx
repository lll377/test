<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="CustomerTypeSetting.aspx.cs" Inherits="M_Main.HouseNew.CustomerTypeSetting" %>
...
<body>
    <form id="FrmForm" method="post" runat="server">
        ...
        <div id="SearchDlg" class="easyui-dialog" title="筛选条件" data-options="iconCls:'icon-search',closed: true,modal:true"
            style="width: 950px; height: 250px; padding: 10px;">
            <table style="width: 100%; padding-top: 5px; line-height: 25px;">

                <tr>
                    <td class="TdTitle">管理项目</td>
                    <td class="TdContent">
                        <input id="CommName" disabled="disabled" maxlength="50" size="10" name="CommName" runat="server">
                        <input id="btnSelComm" class="Button_sel" onclick="btnSelComm_OnClick();" type="button" name="btnSelComm" runat="server">
                    </td>
...
<script type="text/javascript">
    ...
    {
        if ($('#CustTypeID2').val() != '') {
            $.tool.DataPostChk('FrmForm', 'Customer', "SaveCustType", $('#FrmForm').serialize(),
                      function Init() {
                      },
    ...
    function LoadList() {
        ...
        $("#TableContainer").datagrid({
            ...
            onCheck: function (rowIndex, rowData) {
                if ($("#AllData").val().indexOf(rowData.CustID) < 0) {
                    $("#AllData").val($("#AllData").val() + "," +JSON.stringify(rowData));
                }
            },
            onUncheck: function (rowIndex, rowData) {
                $('#IsSelectAll').val(0);

                if ($('#AllData').val().indexOf(rowData.CustID) > -1) {
                    $('#AllData').val($('#AllData').val().replace("," + JSON.stringify(rowData), ''));
                }
            },
            onCheckAll: function (rows) {
                ...
                $.each(rows, function (index, item) {
                    if ($('#AllData').val().indexOf(item.CustID) < 0)
                    {
                        $('#AllData').val($('#AllData').val() + "," + JSON.stringify(item));
                    }
                });
            },
            onUncheckAll: function (rows) {
                ...
                $.each(rows, function (index, item) {
                    if ($('#AllData').val().indexOf(item.CustID) > -1) {
                        $('#AllData').val($('#AllData').val().replace("," + JSON.stringify(item), ''));
                    }
                });
            },
