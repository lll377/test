<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="IncidentAssistManage.aspx.cs" Inherits="M_Main.IncidentNewJH.IncidentAssistManage" %>
...
<body>
    <form id="frmForm" runat="server">
        ...
        <script type="text/javascript">
            ...
            var column = [[
                ...
                { field: 'RoomSign', title: '房屋编号', align: 'center', width: 100, sortable: true },
                {
                    field: 'Title', title: '标题', align: 'left', width: 300, sortable: true, formatter: function (value, row, index) {
                        var str = "<a style=\"color:blue\" onclick=\"Detail('" + row.IID + "','" + row.IncidentID + "','" + row.UserCode + "','edit')\" href=\"javascript:void(0);\">" + row.Title + "</a>";
                        return str;
                    }
                },
                { field: 'AssistReason', title: '协助原因', align: 'left', width: 300, sortable: true },
                { field: 'AssistPeopleCount', title: '申请协助人数', align: 'left', width: 80, sortable: true },
                { field: 'UserName', title: '发起人', width: 100, align: 'center', sortable: true },
                {
                    field: 'WorkStartDate', title: '发起时间', width: 150, align: 'center', sortable: true
                },
                { field: 'FeesRoleNamesMemo', title: '审核流程', width: 220, align: 'left', sortable: true },
                { field: 'AuditState', title: '审核状态', width: 80, align: 'center', sortable: true },
                //{ field: 'FeesRoleNames', title: '未审岗位', width: 100, align: 'left', sortable: true },
                //{
                //    field: 'IID', title: '审核状态', width: 180, align: 'left', sortable: true, formatter: function (value, row, index) {
                //        var values = "";
                //        //已审核数
            ...
            var toolbar = [
                ...
                , '-',
                {
                    ...
                    handler: function () {
                        ...
                        if (rows == null) {
                            HDialog.Info('请选择需要审核的协助申请!');
                        }
                        else {
                            Detail(rows.IID, rows.IncidentID,rows.UserCode, "edit");
                        }
                    }
                }
            ];
            var flag = "1";
            //加载列表 Tb_HSPR_DealyApplay
            function LoadList() {
                $("#TableContainer").datagrid({
                    ...
                    onBeforeLoad: function (param) {
                        param = $.JQForm.GetParam("IncidentProcessingNew", "GetIncidentAssistList", "TableContainer", param);
                        if (flag == "1") {
            ...
            //新增 修改
            function Detail(IID, IncidentID,UserCode, OpType) {
                var w = 1000;
                var h = 300;
                HDialog.Open(w, h, '../IncidentNewJH/IncidentAssistManageDetail.aspx?id=' + IID + '&IncidentID=' + IncidentID + '&OpType=' + OpType + '&DealUserCode=' + UserCode, "报事协助申请", true, function callback(_Data) {
                    if (_Data != '') {
                        $('#SearchDlg').parent().appendTo($("form:first"));
                        LoadList();
                    }
