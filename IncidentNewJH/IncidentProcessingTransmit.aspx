<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="IncidentProcessingTransmit.aspx.cs" Inherits="M_Main.IncidentNewJH.IncidentProcessingTransmit" %>
...
<body>
    <form id="frmForm" runat="server">
        ...
        <input id="hiTypeName" name="hiTypeName" type="hidden" runat="server" />
        ...
        <table class="DialogTable" style="line-height: 25px;">
            ...
            <tr>
                ...
                <td class="TdContent Coorganizer">
                    <input name="CoorganizerUserList" type="text" id="CoorganizerUserList" onclick="ExaCoorganizer()" style="width: 76%;" />
                    <input name="DealManCheckedUserLists" type="hidden" id="DealManCheckedUserLists" disabled="disabled" readonly="readonly" />
                    <input name="CoorganizerUser" type="hidden" id="CoorganizerUser" />
                    <input name="CoorganizerUserCode" type="hidden" id="CoorganizerUserCode" />
                </td>
            </tr>
            <tr>
            ...
            <tr>
                <td style="text-align: center; padding-top: 10px;" colspan="4">
                    ...
                    <input class="button" type="button" value="清空" id="btnClear" name="btnClear" runat="server" />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
...
<script type="text/javascript">
    ...
    $("#btnClear").click(function () {
        ...
        $("#DealMan").searchbox("setValue", '');
        $("#UserCodes").val('');
        $("#CoorganizerUserList").val('');
        $("#DealManCheckedUserLists").val('');
        $("#CoorganizerUser").val('');
        $("#CoorganizerUserCode").val('');
    });
        ...
        if ($("#TypeName").searchbox('getValue') == "") {
            ...
            HDialog.Open('740', '480', '../DialogNew/IncidentDealManChecked.aspx?IsTransmit=0&BigCorpTypeID=' + BigCorpTypeID + '&CheckedUserLists=' + $("#DealManCheckedUserLists").val() + '&IncidentID=' + $("#IncidentID").val(), '选择协助人', false,
                function callback(_Data) {
                    ...
                    if ($('#AssistNames').val() != '') {
                        username = $('#AssistNames').val() + ',' + username;
                    }
                    if ($('#AssistPersonIDs').val() != '') {
                        usercode = $('#AssistPersonIDs').val() + ',' + usercode;
                    }
    ...
    $("#btnSave").click(function () {
        ...
        $.tool.DataPost('IncidentCheckPost', "CheckIncidentTypeAssignedPost2", $('#frmForm').serialize(),
            function Init() {
            },
            function callback(_Data) {
                var data = _Data;
    ...
    ///验证是否有转发记录
    function tranbook() {
        $.tool.DataPost('IncidentProcessingNew', "tranbook", $('#frmForm').serialize(),
            function Init() {
            },
            function callback(_Data) {
                var data = _Data;
