<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="List.aspx.cs" Inherits="M_Main.Renting.Contract.Register.List" %>
        ...
        var toolbar = [
            ...
            }, '-',
            {
                ...
                handler: function () {
                    ...
                    if (row.AuditStatus == "已审核" || row.AuditStatus == "审核中") {
                        $.messager.alert("温馨提示", '当前租赁合同不允许删除!');
                        return;
                    }
                    if (row.IsRentOut == "是") {
                        $.messager.alert("温馨提示", '已退租的租赁合同不允许删除!');
                        return;
                    }
                    $.messager.confirm('确定', '是否删除选中的租赁合同?', function (r) {
                        if (r) {
                            $.dataPostJson('Renting_Contract', 'Del', { "idList": row.ID }, true, false,
                                function Init() {
                                },
                                function callback(data) {
                                    if (data.result) {
                                        $.messager.alert("温馨提示", "删除租赁合同成功！");
                                        InitDataGrid();
            ...
            }, '-',
            {
                ...
                handler: function () {
                    ...
                    if (row) {
                        if (row.IsRentOut == "是") {
                            $.messager.alert("温馨提示", '已退租的租赁合同不允许变更!');
                            return;
                        }
                        var param = { ID: row.ID };
                        LayerDialog.OpenWin('450', '400', '/HM/M_Main/Renting/Contract/Register/Update.aspx?' + $.param(param), "变更合同信息", false, function callback(_Data) {
                            if (_Data != "") {
            ...
            }, '-',
            {
                ...
                handler: function () {
                    var row = $("#TableContainer").datagrid("getSelected");
                    if (row) {
                        var param = { "CustID": row.TenantryCustID };
                        LayerDialog.OpenWin('1000', '500', '/HM/M_Main/HouseNew/CustomerChangeNameBrowse.aspx?' + $.param(param), "更名历史", false, function callback(data) {
                        });
                    } else {
        ...
        function InitDataGrid() {
            $("#TableContainer").datagrid({
                ...
                onBeforeLoad: function (param) {
                    ...
                    param.SortOrder = 1;
                    param.IsDelete = 0;
                    param = $.getDataGridParam("Renting_Contract", "GetList", param);
                },
