<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="List.aspx.cs" Inherits="M_Main.Renting.Merchants.RentSubscription.List" %>
        ...
        <form id="formSearch" class="form-horizontal" role="form">
            <fieldset class="frame-fieldset">
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-3 col-xs-3">
                        <input id="TenantryLegalPeople" name="TenantryLegalPeople" class="easyui-textbox" data-options="tipPosition:'bottom',width:'100%'" />
                    </div>
        ...
        var toolbar = [
            ...
            }, '-',
            {
                ...
                handler: function () {
                    ...
                    if (row.AuditStatus == "已审核" || row.AuditStatus == "审核中") {
                        $.messager.alert("温馨提示", '当前认租签约不允许删除!');
                        return;
                    }
                    if (row.IsFormalSignUp == "是") {
                        $.messager.alert("温馨提示", '认租签约已签署正式合同，不允许删除!');
                        return;
                    }
                    $.messager.confirm('确定', '是否删除选中的认租签约信息?', function (r) {
                        if (r) {
                            $.dataPostJson('Renting_RentalContract', 'Del', { "idList": row.ID }, true, false,
                                function Init() {
                                },
                                function callback(data) {
                                    if (data.result) {
                                        $.messager.alert("温馨提示", "删除认租签约成功！");
                                        InitDataGrid();
            ...
            }, '-',
            {
                ...
                handler: function () {
                    ...
                    if (row) {
                        var param = { ID: row.ID };
                        LayerDialog.OpenWin('450', '400', '/HM/M_Main/Renting/Merchants/RentSubscription/UpdateSignUpTimeAndAddress.aspx?' + $.param(param), "变更认租签约信息", false, function callback(_Data) {
                            if (_Data != "") {
                        ...
                        $.messager.alert("温馨提示", '请选择一行需要变更的数据!');
                    }
                }
            }, '-',
            {
                ...
                handler: function () {
                    ...
                    if (row) {
                        var h = $(window).height();
                        var param = {
                            CustID: row.TenantryCustID,
                            CustName: row.TenantryCustName,
                            PrecAmount: row.DepositAmount,
                            RoomID: "",
                            RoomSign: "",
                            CostID: row.DepositCostItemID,
                            CostName: row.DepositCostItemName
                        };
                        LayerDialog.OpenWin('1200', h, '/HM/M_Main/ChargesNew/PreCostsFrame.aspx?' + $.param(param), "收取定金(预存收款-标准模式)", false, function callback(_Data) {
                            if (_Data != "" && _Data != "1") {
                                $.messager.alert('温馨提示', "收取定金成功！");
                            }
                        });
                    } else {
                        $.messager.alert("温馨提示", '请选择一行需要收取定金的数据!');
                    }
                }
            }, '-',
            {
                ...
                handler: function () {
                    ...
                    if (row) {
                        if (row.AuditStatus != "已审核") {
                            $.messager.alert("温馨提示", '当前认租签约不允许正式签约!');
                            return;
                        }
                        if (row.IsFormalSignUp == "是") {
                            $.messager.alert("温馨提示", '认租签约已签署正式合同，不允许重复签约!');
                            return;
                        }
                        var param = { "OpType": 'add', "ID": "", "SignType": "新签", "RentalContractID": row.ID };
                        var w = '1200';
                        var h = $(window).height();
            ...
            }, '-',
            {
                ...
                handler: function () {
                    ...
                    if (row) {
                        if (row.AuditStatus != "已审核") {
                            $.messager.alert("温馨提示", '当前认租签约不允许作废!');
                            return;
                        }
                        if (row.IsFormalSignUp == "是") {
                            $.messager.alert("温馨提示", '当前认租签约已签署正式合同，不允许作废!');
                            return;
                        }
                        $.messager.confirm('确定', '是否作废选中的认租签约?', function (r) {
                            if (r) {
                                $.dataPostJson('Renting_RentalContract', 'Abandonment', { "idList": row.ID }, true, false,
                                    function Init() {
                                    },
                                    function callback(data) {
                                        if (data.result) {
                        ...
                        $.messager.alert("温馨提示", '请选择一行需要作废的认租协议!');
                    }
                }
            }
        ];
