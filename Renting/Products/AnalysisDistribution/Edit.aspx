<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Edit.aspx.cs" Inherits="M_Main.Renting.Products.AnalysisDistribution.Edit" %>
...
<head runat="server">
    ...
    <title>租赁管理-竞品管理-竞品分布分析-距离分组设置</title>
...
<body style="padding: 0px; margin: 0px; overflow: hidden;">
    ...
    <script type="text/javascript">
        ...
        var toolbar = [
            ...
            {
                iconCls: 'icon-help',
                text: '1、不填写开始距离【(结束距离)以内】2、不填写结束距离【(开始距离)以上】',
                handler: function () {
                }
            }
        ];
        var column = [[
            ...
            {
                field: 'StartingDistance', title: '开始距离(公里)≤[距本项目距离]', width: 100, align: 'center', editor: {
                    type: 'numberbox',
                    options: {
                        min: 0,
                        max: 10000,
            ...
            {
                field: 'EndDistance', title: '结束距离(公里)＞[距本项目距离]', width: 100, align: 'center', editor: {
                    type: 'numberbox',
                    options: {
                        min: 0,
                        max: 10000,
        ...
        function InitDataGrid() {
            $("#TableContainer").datagrid({
                ...
                onBeforeLoad: function (param) {
                    ...
                    param.SortOrder = 0;
                    param.IsDelete = 0;
                    param = $.getDataGridParam("Renting_Products_AnalysisDistribution", "GetList", param);
                },
        ...
        function Save() {
            ...
            for (var i = 0; i < Rows.length; i++) {
                ...
                if (Rows[i]["StartingDistance"] == "" && Rows[i]["EndDistance"] == "") {
                    $.messager.alert('温馨提示', "第【" + (parseInt(i) + 1) + "】行开始距离、结束距离不允许都为空！");
                    return;
                }
                //验证开始是否大于结束
                if (parseFloat(Rows[i]["StartingDistance"]) >= parseFloat(Rows[i]["EndDistance"])) {
                    $.messager.alert('温馨提示', "第【" + (parseInt(i) + 1) + "】行开始距离不允许大于等于结束距离！");
                    return;
                }
            }
            var param = $.getParam();
            param.DetailResult = JSON.stringify(Rows);
            $.dataPostJson('Renting_Products_AnalysisDistribution', 'Save', param, true, false,
                function Init() {
                },
