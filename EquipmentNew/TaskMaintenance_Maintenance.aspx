<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="TaskMaintenance_Maintenance.aspx.cs" Inherits="M_Main.EquipmentNew.TaskMaintenance_Maintenance" %>
        ...
        <div id="SearchDlg" class="easyui-dialog" title="筛选条件" data-options="iconCls:'icon-search',closed: true,modal:true" style="width: 1000px; height: 270px; overflow: hidden; padding: 10px;">
            <table style="width: 100%; font-size: 12px; font-family: 微软雅黑;" class="SearchTable">
                ...
                <tr>
                    ...
                    <td class="TdContentSearch">
                        <select id="IsCheck" name="IsCheck" style="width: 128px; border: 1px #cccccc solid; height: 22px;" class="easyui-validatebox" runat="server">
                            <option value=""></option>
                            <option value="0">否</option>
                            <option value="1">是</option>
                        </select>
                    </td>
                    <td class="TdTitle">验收结果</td>
                    <td class="TdContentSearch">
                        <select id="CheckResult" name="CheckResult" style="width: 128px; border: 1px #cccccc solid; height: 22px;" class="easyui-validatebox" runat="server">
                            <option value=""></option>
        ...
        <script type="text/javascript">
            ...
            var toolbar = [{
                ...
                handler: function () {
                    if ($("#CommId").val().length < 3) {
                        ...
                        HDialog.Open('1100', '510', '../EquipmentNew/TemporaryPlanGeneration_Maintenance.aspx', '任务维护', true, function callback(_Data) {
            ...
            }, '-',
            {
                ...
                handler: function () {
                    ...
                    window.open("TaskMaintenanceExcel_Maintenance.aspx?random=" + Math.random() + $('#frmFrom').serialize());
                }
            }, '-',
            {
                text: '筛选',
            ...
            //修改
            function Edit(id) {
                HDialog.Open('720', '250', '../EquipmentNew/TaskMaintenanceEdit_Maintenance.aspx?OpType=Edit&TaskId=' + id + '&CommId=' + $("#CommId").val(), '修改', true, function callback(_Data) {
                    LoadList();
                });
            }
            ...
            function DelRecord(TaskId) {
                $.messager.confirm('确定', '是否确定删除！', function (r) {
                    if (r) {
                        $.tool.DataGet('CsEquipment', 'DelTaskMaintenance_Maintenance', 'TaskId=' + TaskId + '&CommId=' + $("#CommId").val(),
                            function Init() {
                            },
            ...
            function Close(Id) {
                if (Id != null) {
                    ...
                    HDialog.Open('720', '250', '../EquipmentNew/TaskMaintenanceClose_Maintenance.aspx?TaskId=' + Id + '&CommId=' + $("#CommId").val(), '关闭', true, function callback(_Data) {
            ...
            function LoadList() {
                $("#TableContainer").datagrid({
                    url: '/HM/M_Main/HC/DataGetControl.aspx?Method=DataGet&Class=CsEquipment&Command=GetPlanGenerationDetailsList_Maintenance&' + $('#frmFrom').serialize(),
                    method: "get",
                    title: '',
                    loadMsg: '数据加载中,请稍候...',
                    nowrap: false,
