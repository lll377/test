<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="TaskMaintenanceEdit_Maintenance.aspx.cs" Inherits="M_Main.EquipmentNew.TaskMaintenanceEdit_Maintenance" %>
    ...
    <form id="frmFrom" runat="server">
        <table class="SearchTable" style="width: 100%; padding-top: 5px; line-height: 25px;">
            ...
            <tr>
                ...
                <td class="TdContentSearch">
                    <input type="text" id="NewTaskUserName" name="NewTaskUserName" runat="server" data-options="required:true" style="border: 1px solid #cccccc; width: 80%;" class="easyui-validatebox" onclick="SelTaskUser();" />
        ...
        <script type="text/javascript">
            function Load() {
                $.tool.DataGet('CsEquipment', 'SelTaskMaintenanceEdit_Maintenance', $('#frmFrom').serialize(),
                    function Init() {
                    },
                    function callback(_Data) {
                        ...
                        if (varObjects.length > 0) {
                            if (varObjects.length < 2) {
                                ...
                                $("#BeginTime").val(formatDate(varObjects[0].TaskBeginTime, "yyyy-MM-dd HH:mm:ss"));
                                $("#EndTime").val(formatDate(varObjects[0].TaskEndTime, "yyyy-MM-dd HH:mm:ss"));
                                $("#NewBeginTime").val(formatDate(varObjects[0].TaskBeginTime, "yyyy-MM-dd HH:mm:ss"));
                                $("#NewEndTime").val(formatDate(varObjects[0].TaskEndTime, "yyyy-MM-dd HH:mm:ss"));
                            }
                        } else {
                            HDialog.Close();
                        }
            ...
            function Save() {
                ...
                $.tool.DataPostChk('frmFrom', 'CsEquipment', 'SaveTaskMaintenanceEdit_Maintenance', $('#frmFrom').serialize(),
                    function Init() {
                    },
                    function callback(_Data) {
                        HDialog.Info("修改成功!");
            ...
            $("#SeeHistory").click(function () {
                HDialog.Open('800', '500', '../EquipmentNew/TaskMaintenanceEditHistory_Maintenance.aspx?TaskId=' + $("#TaskId").val() + '&CommId=' + $("#CommId").val(), '查看历史修改记录', true, function callback(_Data) {
