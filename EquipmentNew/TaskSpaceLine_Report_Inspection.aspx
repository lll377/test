<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="TaskSpaceLine_Report_Inspection.aspx.cs" Inherits="M_Main.EquipmentNew.TaskSpaceLine_Report_Inspection" %>
...
<body>
    <form id="frmForm" runat="server">
        ...
        <input type="hidden" id="TaskName" name="TaskName" value="设备巡检表单" />
        ...
        <script type="text/javascript">
            ...
            var column = [[
                ...
                { field: 'UserName', title: '巡检人', width: 120, align: 'left', sortable: true }
            ]];
            ...
            function GetColumn() {
                $.tool.DataGet('CsEquipment', 'GetTaskSpaceLine_Report_TitleList_Inspection', 'CommId=' + $('#CommId').val() + '&SpaceId=' + $("#SpaceId").val() + '&BeginTime=' + $('#BeginTime').val() + '&EndTime=' + $('#EndTime').val(),
                    function Init() {
                    },
                    function callback(_Data) {
                        ...
                        for (var i = 0; i < varObjects.length; i++) {
                            column[0].push({ field: varObjects[i].InspectionStandardDetailId, title: varObjects[i].InspectionContent.replace(re, '').replace(re1, '').replace(re2, ''), width: 200, align: 'left', sortable: true });
                        }
                        GetList('1');
                    },
            ...
            function GetList(str) {
                $("#TableContainer").datagrid({
                    ...
                    onBeforeLoad: function (param) {
                        if (str != "") {
                            param = $.JQForm.GetParam("CsEquipment", "GetTaskSpaceLine_ReportList_Inspection", "TableContainer", param);
                        }
                    }
                });
            ...
            //导出Excel
            function Export() {
                $.tool.DataGet('CsEquipment', 'GetTaskSpaceLine_ReportList_Inspection', 'CommId=' + $('#CommId').val() + '&SpaceId=' + $("#SpaceId").val() + '&BeginTime=' + $('#BeginTime').val() + '&EndTime=' + $('#EndTime').val() + '&page=1&rows=999999999',
                    function Init() {
                    },
                    function callback(_Data) {
                        ...
                        tableString += $('#CommName').val() + '-' + $('#SpaceName').val() + '设备巡检单';
                        tableString += '</td>';
                        ...
                        for (var i = 0; i < rows.length; i++) {
                            ...
                            for (var j = 0; j < varObjects.length; j++) {
                                tableString += '\n<td>';
                                var FiledName = varObjects[j].InspectionStandardDetailId;
                        ...
                        i.appendTo(f);
                        var date = new Date();
                        l.val($("#SpaceName").val() + '机房巡检计划输出表单-' + dateFormat("YYYY-mm-dd", date));
                        l.appendTo(f);
