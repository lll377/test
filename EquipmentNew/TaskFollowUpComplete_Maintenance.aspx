<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="TaskFollowUpComplete_Maintenance.aspx.cs" Inherits="M_Main.EquipmentNew.TaskFollowUpComplete_Maintenance" %>
...
<body class="easyui-layout" style="margin: 0px; padding: 0px; overflow: hidden;">
    ...
    <div class="tabs-container">
        <ul class="nav nav-tabs">
            <li class="active" name="room" refsel="1"><a data-toggle="tab" href="#tab-1" aria-expanded="true">空间维保</a>
            </li>
            <li class="" name="cust" refsel="0"><a data-toggle="tab" href="#tab-2" aria-expanded="false">设备维保</a>
            </li>
        </ul>
        ...
        <form id="frmFrom">
            <table class="SearchTable">
                ...
                <tr>
                    ...
                    <td class="TdContentSearch">
                        <input type="text" id="MaintenanceContent" name="MaintenanceContent" maxlength="1000" class="easyui-textbox" />
                    </td>
    ...
    <input type="hidden" id="Flag" name="Flag" value="Complete" runat="server" />
    <script type="text/javascript">
            ...
            [
                        ...
                        if (row != null && row != "" && row != undefined) {
                            if (row.length > 0) {
                                for (var i = 0; i < row.length; i++) {
                                    if (row[i].IsCheck == "1") {
                                        HDialog.Info("勾选计划中有已验收计划，不能重复验收。");
                                        return;
                                    }
                                    TaskId += "," + row[i].TaskId;
                                }
                ...
                }, '-',
                {
                    ...
                    handler: function () {
                        ...
                        window.open("TaskFollowUpExport.aspx?Type=Maintenance&Flag=" + $('#Flag').val() + "&CommID=" + $('#CommID').val() + "&" + $("#frmFrom").serialize());
                    }
                }, '-',
                {
                    text: '筛选',
        ...
        var column = [[
            ...
            { field: 'MaintenanceContent', title: '维保内容', width: 200, align: 'left', sortable: true },
            ...
            { field: 'ExecuteUserName', title: '维保人', width: 60, align: 'left', sortable: true },
            { field: 'ExecuteBeginTime', title: '维保开始时间', width: 145, align: 'left', sortable: true },
            { field: 'ExecuteEndTime', title: '维保结束时间', width: 145, align: 'left', sortable: true },
            { field: 'MaintenanceHours', title: '维保工时', width: 80, align: 'left', sortable: true },
            {
                field: 'IsCheck', title: '是否验收', width: 80, align: 'left', sortable: true, formatter: function (value, rows) {
                    var str = "";
                    if (rows.IsCheck == 1) {
                        str = "是"
                    } else
            ...
            { field: 'CheckUser', title: '验收人', width: 80, align: 'left', sortable: true },
            {
                field: 'CheckTime', title: '验收时间', width: 150, align: 'left', sortable: true, formatter: function (value, row, index) {
                    var str = formatDate(row.CheckTime, "yyyy-MM-dd");
                    return str;
                }
            },
            { field: 'CheckResultName', title: '验收结果', width: 100, align: 'left', sortable: true },
            { field: 'CheckRemark', title: '验收意见', width: 250, align: 'left', sortable: true }
        ]];
        ...
        function LoadBind() {
            $("#DivContainer").datagrid({
                ...
                onBeforeLoad: function (param) {
                    param = $.JQForm.GetParam("CsEquipment", "GetTaskFollowUpList_Maintenance", "DivContainer", param);
                },
        ...
        var columnDetail = [[
            ...
            { field: 'ResultDescribe', title: '维保描述', width: 60, align: 'left', sortable: true },
            { field: 'Result', title: '维保结果', width: 25, align: 'left', sortable: true },
            {
        ...
        function Close(Id) {
            if (Id != null) {
                $.tool.DataGet('CsEquipment', 'TaskMaintenanceCheck_Maintenance', 'TaskId=' + Id + '&CommId=' + $("#CommID").val(),
                    function Init() {
                    },
                    function callback(_Data) {
                        if (_Data == "1") {
                            HDialog.Open('720', '250', '../EquipmentNew/TaskMaintenanceCheck_Maintenance.aspx?TaskId=' + Id + '&CommId=' + $("#CommID").val(), '验收', true, function callback(_Data) {
                                LoadBind();
                            });
                        } else {
                            HDialog.Info("有部分任务您无验收权限，请重新选择");
                        }
                    },
        ...
        function checkPatrolMsgDetail(EquipmentId, TaskId, CommID) {
            HDialog.Open('840', '450', '../EquipmentNew/TaskFollowUpLineDetail_Maintenance.aspx?EquipmentId=' + EquipmentId + '&TaskId=' + TaskId + "&CommID=" + CommID, '设备工艺路线详情', true, function callback(_Data) {
            });
        }
        //查看详情
        function checkPatrolMsgDetailSpace(SpaceId, TaskId, CommID) {
            HDialog.Open('840', '450', '../EquipmentNew/TaskFollowUpLineDetail_Maintenance_Space.aspx?SpaceId=' + SpaceId + '&TaskId=' + TaskId + "&CommID=" + CommID, '空间工艺路线详情', true, function callback(_Data) {
            });
        }
        ...
        function LoadBindDetail(Id) {
            ...
            $("#DivContainerDetail").datagrid({
                ...
                onBeforeLoad: function (param) {
                    ...
                    param.Command = "GetTaskFollowUpEquipmentList_Maintenance";
                    param.TaskId = TaskId;
        ...
        function LoadBindDetailSpace(Id) {
            ...
            $("#DivContainerDetailSpace").datagrid({
                ...
                onBeforeLoad: function (param) {
                    param.Method = "DataPost";
                    param.Class = "CsEquipment";
                    param.Command = "GetTaskFollowUpSpaceList_Maintenance";
                    param.TaskId = TaskId;
                    param.CommID = $('#CommID').val();
                }
            });
