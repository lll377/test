<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="PlanGeneration_Inspection.aspx.cs" Inherits="M_Main.EquipmentNew.PlanGeneration_Inspection" %>
    ...
    <form id="frmForm" runat="server">
        ...
        <script type="text/javascript">
            ...
            function ShowPatralPlanDetails(TaskProduceId, AddPId) {
                HDialog.Open('1400', '600', '../EquipmentNew/PlanGenerationComm_Inspection.aspx?TaskProduceId=' + TaskProduceId + '&AddPId=' + AddPId, '任务批次项目列表', true, function callback(_Data) {
                    LoadList();
                });
            }
            //function SelUser() {
            ...
            var toolbar = [
                {
                    ...
                    iconCls: 'icon-add',
                    handler: function () {
                        HDialog.OpenWinNoClose('800', '285', '../EquipmentNew/PlanGenerationAdd_Inspection.aspx?IsFireControl=' + $("#IsFireControl").val(), '任务生成', true, function callback(_Data) {
                            LoadList();
                            if (_Data == "2") {//生成有错误  展示错误信息
                                HDialog.Open('1400', '600', '../EquipmentNew/PlanGenerationProblem_Inspection.aspx', '生成信息', true, function callback(_Data) {
                                });
                            }
                        });
                    }
            ...
            function IsTaskProduceImplement(str) {
                var IsBool = false;
                $.ajax({
                    url: '/HM/M_Main/HC/DataGetControl.aspx?Method=DataGet&Class=CsEquipment&Command=IsTaskProduceImplement_Inspection&TaskProduceId=' + str,
                    type: 'GET',
                    dataType: 'text',
                    cache: false,
                    async: false,
            ...
            function DelRecord(TaskProduceId) {
                $.tool.DataGet('CsEquipment', 'DelTaskProduce_Inspection', 'TaskProduceId=' + TaskProduceId,
                    function Init() {
                    },
            ...
            function LoadList() {
                ...
                $("#TableContainer").datagrid({
                    url: '/HM/M_Main/HC/DataGetControl.aspx?Method=DataGet&Class=CsEquipment&Command=GetTaskProduceList_Inspection&' + $('#frmForm').serialize(),
                    method: "get",
                    loadMsg: '数据加载中,请稍候...',
                    nowrap: false,
