<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="TaxRateSettingChange.aspx.cs" Inherits="M_Main.ChargesNew.TaxRateSettingChange" %>
...
<body>
    <form id="frmForm" runat="server">
        ...
        <input type="hidden" id="hidBool" name="hidBool" value="" />
        <script type="text/javascript">
            ...
            function load() {
                if ($("#OpType").val() == 'insert') {
                    document.getElementById("StartDate").onclick = function (e) {
                        ...
                        now.setDate(now.getDate() + 1);
                        var strMinDate = now;
                        WdatePicker({ dateFmt: 'yyyy-MM-dd', minDate: '%y-%M-01' });
                    };
                }
            }
            load();
            ...
            function SelCost() {
                ...
                HDialog.Open('700', '450', "../dialogNew/MultiCorpCommCostDlgNoParentNode.aspx" + "?Ram=" + Math.random() + "&CommID=" + $("#CommID").val(), '收费项目选择', false, function (_data) {
                    var varReturn = _data;
                    ...
                    function callback(_Data) {
                        ...
                        if (varObj.length > 0) {
                            var strObj = varObjects[0].split(',');
                            if (varObj.length < strObj.length) {
                                HDialog.Info("当前项目发现不存在的费用科目!");
                            }
                            else {
                                var strSign = 0;
                                var strSignT = 0;
            ...
            function Load() {
                if ($("#OpType").val() == 'edit') {
                    ...
                    $.tool.DataGet('TaxRateSetting', 'GetModel', $('#frmForm').serialize(),
                        function Init() {
                        },
                        function callback(_Data) {
