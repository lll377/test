<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="TaskExpired_Ambient.aspx.cs" Inherits="M_Main.ComprehensivePatrols.TaskExpired_Ambient" %>
    ...
    <form id="frmFrom" runat="server">
        ...
        <script type="text/javascript">
            ...
            var toolbarDetail =
                [{
                    ...
                    handler: function () {
                        ...
                        formData.append("TaskName", '环境任务点位');
                        $.ajax({
                            type: "POST",
            ...
            var columnDetail = [[
                ...
                {
                    field: 'IsMustCheck', title: '是否必查', width: 70, align: 'left', sortable: false, formatter: function (value, row, index) {
                        var str = "";
                ...
                { field: 'DidUserName', title: '扫码人', width: 80, align: 'left', sortable: false },
                { field: 'DidTime', title: '扫码时间', width: 100, align: 'left', sortable: false },
                {
                ...
                {
                    field: 'fj', title: '任务附件', width: 80, align: 'left', sortable: true, formatter: function (value, row, index) {
            ...
            function OnFileList(TaskId, TaskPointId) {
                HDialog.Open(600, 450, '../ComprehensivePatrols/ComprehensivePatrolsFiles_Ambient.aspx?TaskId=' + TaskId + "&TaskPointId=" + TaskPointId, '查看附件', true, function callback(_Data) {
                });
            }

            function checkPatrolMsg(TaskId, TaskPointId) {
                HDialog.Open('900', '600', '../ComprehensivePatrols/CpPlanTaskStandard_Ambient.aspx?TaskPointId=' + TaskPointId + "&TaskId=" + TaskId + "&CommId=" + $("#CommId").val(),
                    '对象/标准', true, function callback(_Data) {
                    });
            }
            //查看二维码
            function ShowQRCode(pointId, pointCode, pointName, addr) {
                HDialog.Open('360', '400', '../ComprehensivePatrols/PointShowQrCode_Ambient.aspx?PointId=' + pointId + "&PointCode=" + escape(pointCode) + "&PointName=" + escape(pointName) + "&Addr=" + escape(addr), '查看二维码', true, function callback(_Data) {
                });
            }

            //查看计划轨迹
            function checkPlanTrajectory(TaskId, PlanId, CommId) {
                HDialog.Open('1100', '600', '../ComprehensivePatrols/TaskRouteFrame_Ambient.aspx?TaskId=' + TaskId + '&PlanId=' + PlanId + '&CommId=' + CommId,
            ...
            function ShowMap(longitude) {
                HDialog.Open('700', '540', '../ComprehensivePatrols/PointShowMap_Ambient.aspx?longitude=' + longitude, '查看地图', true, function callback(_Data) {
                });
            }
            //查看报事
            function TaskPointIncident(TaskPointId, TaskId) {
                HDialog.Open(800, 600, '../ComprehensivePatrols/TaskPointIncident_Ambient.aspx?TaskId=' + TaskId + "&TaskPointId=" + TaskPointId + "&CommId=" + $("#CommId").val(), '查看报事', true, function callback(_Data) {
                });
            }
            ...
            function LoadList() {
                ...
                $("#DivContainer").datagrid({
                    url: '/HM/M_Main/HC/DataGetControl.aspx?Method=DataGet&Class=CpComPatrols&Command=GetCpPlanTaskList_Ambient&Flag=已过期&' + $("#frmFrom").serialize(),
                    method: "get",
                    loadMsg: '数据加载中,请稍候...',
                    nowrap: false,
                    pageSize: 20,
            ...
            function LoadListDetail(Id) {
                $("#DivContainerDetail").datagrid({
                    ...
                    onBeforeLoad: function (param) {
                        ...
                        param.Command = "GetCpPlanTaskPointList_Ambient";
                        param.TaskId = Id;
