<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="PlanTemplatePoint.aspx.cs" Inherits="M_Main.ComprehensivePatrols.PlanTemplatePoint" %>
    ...
    <a href="#" class="easyui-linkbutton" onclick="SetPercentage();" data-options="iconCls:'icon-save',plain:true">设置覆盖率</a><span style="color: red">提示：没有设置抽查点位覆盖率，不能审核计划</span>
        ...
        <div id="SearchDlg" class="easyui-dialog" title="筛选条件" data-options="iconCls:'icon-search',closed: true,modal:true" style="width: 700px; height: 160px; padding: 10px;">
            <table style="width: 100%; font-size: 12px; font-family: 微软雅黑;" class="SearchTable">
                ...
                <tr>
                    ...
                    <td class="TdContentSearch">
                        <select id="TaskType" name="TaskType" isdctype='true' dctype="任务类型" dcname="品质核查" class="easyui-validatebox" style="border: 1px #cccccc solid; height: 22px; line-height: 22px; width: 99%">
                        </select>
                    </td>
        ...
        <script type="text/javascript">
            ...
            function SelSysPoint() {
                HDialog.Open('600', '450', '../ComprehensivePatrols/DialogNewSysPoint.aspx?Single=false&Type=Cp&PlanId=' + $("#PlanId").val(), '选择通用点位', true, function callback(data) {
            ...
            function getExcel() {
                //window.open("CpReportExcel.aspx?PlanId=" + $('#PlanId').val() + '&SysPointId=' + $('#SysPointId').val() + '&PointCode=' + $('#PointCode').val() + '&PointName=' + $('#PointName').val() + '&TaskType=' + $('#TaskType').val() + '&Professional=' + $("#Professional").combobox('getValues') + '&DType=' + $("#DType").combobox('getValues') + '&TaskName=品质点位');
            ...
            function DelRecord(Ids) {
                $.tool.DataPost('CpComPatrols', 'DelPlanPoint', 'Ids=' + Ids + '&PlanId=' + $("#PlanId").val(),
                    function Init() {
                    },
            ...
            function SetPercentage() {
                $.tool.DataPost('CpComPatrols', 'HaveOtherPoint', 'PlanId=' + $("#PlanId").val(),
                    function Init() {
                    },
                    function callback(_Data) {
                        var text = '设置'
                        ...
                        HDialog.Open('600', '500', '../ComprehensivePatrols/PlanOtherPointPercentage.aspx?PlanId=' + $("#PlanId").val() + '&CanEdit=' + $("#CanEdit").val(), '抽查点位覆盖率' + text, true, function callback(_Data) {
                            ReloadOtherPointPercentage();
                        });
                    },
                    function completeCallback() {
                ...
                //$.tool.DataPost('CpComPatrols', 'SavePercentage', 'PlanId=' + $("#PlanId").val() + '&OtherPointPercentage=' + $("#OtherPointPercentage").val(),
                //    function Init() {
                //    },
                //    function callback(_Data) {
                //        HDialog.Info("保存成功！");
            ...
            function RefreshPoint() {
                $.tool.DataPost('CpComPatrols', 'RefreshPoint', 'PlanId=' + $("#PlanId").val(),
                    function Init() {
                    },
                    function callback(_Data) {
                        ReloadOtherPointPercentage();
            ...
            function SavePoint() {
                ...
                if (row != null && row != undefined && row != "") {
                    ...
                    $.tool.DataPost('CpComPatrols', 'SetPlanPointGo', 'Ids=' + ids + '&PlanId=' + $("#PlanId").val(),
                        function Init() {
                        },
                        function callback(_Data) {
                            ReloadOtherPointPercentage();
            ...
            function SaveSort() {
                ...
                if (rows != null && rows != undefined && rows != "") {
                    ...
                    Json = '[' + Json + ']';
                    $.tool.DataPost('CpComPatrols', 'SetSortGo', 'PlanId=' + $("#PlanId").val() + '&Json=' + Json,
                        function Init() {
                        },
                        function callback(_Data) {
                            if (_Data > '')
            ...
            function ReloadOtherPointPercentage() {
                if ($("#PlanId").val() > '') {
                    $.tool.DataPost('CpComPatrols', 'ReloadOtherPointPercentage', 'PlanId=' + $("#PlanId").val(),
                        function Init() {
                        },
                        function callback(_Data) {
            ...
            function LoadOtherPointPercentage() {
                if ($("#PlanId").val() > '') {
                    $.tool.DataPost('CpComPatrols', 'GetOtherPointPercentageByPlanId', 'PlanId=' + $("#PlanId").val(),
                        function Init() {
                        },
                        function callback(_Data) {
                            $("#OtherPointPercentage").val(_Data);
            ...
            function LoadList() {
                ...
                $("#TableContainer").datagrid({
                    ...
                    onBeforeLoad: function (param) {
                        param = $.JQForm.GetParam("CpComPatrols", "GetPlanTemplatePoint", "TableContainer", param);
                    }
                });
