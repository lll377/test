<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="PlanTemplateEdit_Patrol.aspx.cs" Inherits="M_Main.ComprehensivePatrols.PlanTemplateEdit_Patrol" %>
    ...
    <form id="frmForm" runat="server">
        <div class="Container">
            <table class="SearchTable">
                <tr>
                    ...
                    <td class="TdContentSearch">
                        <select id="TypeId" name="TypeId" isdctype='true' dctype="任务类型" dcname="公区巡查,空房巡查,装修巡查" class="easyui-validatebox" style="border: 1px #cccccc solid; height: 22px; line-height: 22px; width: 70%;" data-options="required:true" onchange="SelTaskType();SetPlanName();">
                        </select>
                    </td>
        ...
        <script type="text/javascript">
            ...
            function SelTaskType(LevelId,FrequencyTitle) {
                ...
                window.parent.ChangeType($("#TypeId").find("option:selected").text());
            }
            ...
            function Save() {
                ...
                var work = "&FrequencyTitle=" + $("#Frequency").find("option:selected").text() + "&WorkTime=" + WorkTime + "&WorkWeek=" + WorkWeek + "&WorkMonth=" + WorkMonth + "&TypeName=" + $("#TypeId").find("option:selected").text();
            ...
            function SavePlan(work, NeedClear) {
                $.tool.DataPostChk('frmForm', 'CpComPatrols', 'SavePlanTemplate_Patrol', $('#frmForm').serialize() + work + '&NeedClear=' + NeedClear,
                    function Init() {
                    },
                    function callback(_Data) {
                        if (_Data > '') {
            ...
            function LoadData(PlanId) {
                $.tool.DataGet('CpComPatrols', 'GetPlanTemplateList_Patrol', 'page=1&rows=10&PlanId=' + PlanId + '&CommId=' + $("#CommId").val(),
                    function Init() {
                    },
                    function callback(_Data) {
                        varObjects = eval("(" + _Data.replace(/\\/g, "/") + ")"); //转换为json对象
