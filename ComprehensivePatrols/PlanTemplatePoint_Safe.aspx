<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="PlanTemplatePoint_Safe.aspx.cs" Inherits="M_Main.ComprehensivePatrols.PlanTemplatePoint_Safe" %>
    ...
    <a href="#" class="easyui-linkbutton" onclick="SavePercentage();" data-options="iconCls:'icon-save',plain:true">保存覆盖率</a>
        ...
        <div id="SearchDlg" class="easyui-dialog" title="筛选条件" data-options="iconCls:'icon-search',closed: true,modal:true" style="width: 700px; height: 160px; padding: 10px;">
            <table style="width: 100%; font-size: 12px; font-family: 微软雅黑;" class="SearchTable">
                ...
                <tr>
                    ...
                    <td class="TdContentSearch">
                        <select id="TaskType" name="TaskType" isdctype='true' dctype="任务类型" dcname="安全巡更" class="easyui-validatebox" style="border: 1px #cccccc solid; height: 22px; line-height: 22px; width: 99%">
                        </select>
                    </td>
        ...
        <script type="text/javascript">
            ...
            function SelSysPoint() {
                HDialog.Open('600', '450', '../ComprehensivePatrols/DialogNewSysPoint.aspx?Single=false&Type=Safe&PlanId=' + $("#PlanId").val(), '选择通用点位', true, function callback(data) {
            ...
            function getExcel() {
                //window.open("CpReportExcel.aspx?PlanId=" + $('#PlanId').val() + '&SysPointId=' + $('#SysPointId').val() + '&PointCode=' + $('#PointCode').val() + '&PointName=' + $('#PointName').val() + '&TaskType=' + $('#TaskType').val() + '&Professional=' + $("#Professional").combobox('getValues') + '&DType=' + $("#DType").combobox('getValues') + '&TaskName=安全点位');
            ...
            function DelRecord(Ids) {
                $.tool.DataPost('CpComPatrols', 'DelPlanPoint_Safe', 'Ids=' + Ids + '&PlanId=' + $("#PlanId").val(),
                    function Init() {
                    },
            ...
            function SavePercentage() {
                $.tool.DataPost('CpComPatrols', 'SavePercentage_Safe', 'PlanId=' + $("#PlanId").val() + '&OtherPointPercentage=' + $("#OtherPointPercentage").val(),
                    function Init() {
                    },
                    function callback(_Data) {
                        HDialog.Info("保存成功！");
            ...
            function RefreshPoint() {
                $.tool.DataPost('CpComPatrols', 'RefreshPoint_Safe', 'PlanId=' + $("#PlanId").val(),
                    function Init() {
                    },
                    function callback(_Data) {
                        LoadList();
            ...
            function SavePoint() {
                ...
                if (row != null && row != undefined && row != "") {
                    ...
                    $.tool.DataPost('CpComPatrols', 'SetPlanPointGo_Safe', 'Ids=' + ids + '&PlanId=' + $("#PlanId").val(),
                        function Init() {
                        },
                        function callback(_Data) {
                            LoadList();
            ...
            function SaveSort() {
                ...
                if (rows != null && rows != undefined && rows != "") {
                    ...
                    Json = '[' + Json + ']';
                    $.tool.DataPost('CpComPatrols', 'SetSortGo_Safe', 'PlanId=' + $("#PlanId").val() + '&Json=' + Json,
                        function Init() {
                        },
                        function callback(_Data) {
                            if (_Data > '')
            ...
            function LoadOtherPointPercentage() {
                if ($("#PlanId").val() > '') {
                    $.tool.DataPost('CpComPatrols', 'GetOtherPointPercentageByPlanId_Safe', 'PlanId=' + $("#PlanId").val(),
                        function Init() {
                        },
                        function callback(_Data) {
            ...
            function LoadList() {
                ...
                $("#TableContainer").datagrid({
                    ...
                    onBeforeLoad: function (param) {
                        param = $.JQForm.GetParam("CpComPatrols", "GetPlanTemplatePoint_Safe", "TableContainer", param);
                    }
                });
