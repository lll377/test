<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="PlanTemplateList_Patrol.aspx.cs" Inherits="M_Main.ComprehensivePatrols.PlanTemplateList_Patrol" %>
    ...
    <form id="frmFrom" runat="server">
        ...
        <script type="text/javascript">
            ...
            var column = [[
                ...
                { field: 'AuditingRemark', title: '审核备注', width: 300, align: 'left', sortable: true },
                {
            ...
            var toolbar = [
                {
                    ...
                    handler: function () {
                        if ($("#CommId").val().length < 3) {
                            ...
                            HDialog.Open(w, '600', '../ComprehensivePatrols/PlanTemplateFrame_Patrol.aspx', '新增计划模板', true, function callback(_Data) {
                                LoadList();
                                ClearStorage();
                            });
                        }
                ...
                , '-',
                {
                    ...
                    handler: function () {
                        ...
                        if (row != "" && row != null && row != undefined) {
                            ...
                            HDialog.Open(w, '600', '../ComprehensivePatrols/PlanTemplateFrame_Patrol.aspx', '复制计划模板', true, function callback(_Data) {
                                LoadList();
                                ClearStorage();
                            });
                        } else {
                    ...
                    handler: function () {
                        ...
                        if (row != "" && row != null && row != undefined) {
                            if (row.IsAuditing == "1") {
                                ...
                                Examine(row.PlanId, row.LevelId, row.TaskTypeName);
                                //});
                            }
                        }
                        else {
                ...
                , '-',
                {
                    ...
                    handler: function () {
                        ...
                        if (row.length > 0) {
                            ...
                            HDialog.Open('720', '190', '../ComprehensivePatrols/PlanTaskUserEdit_Patrol.aspx?Id=' + str + '&CommId=' + row[0].CommId, '修改', true, function callback(_Data) {
                                LoadList();
                            });
                        } else {
            ...
            function ShowPlanDlg(PlanId, CommId, CommName, AddUserCode, IsAuditing) {
                ...
                HDialog.Open(w, '600', '../ComprehensivePatrols/PlanTemplateFrame_Patrol.aspx', '计划模板详情', false, function callback(_Data) {
                    LoadList();
                    ClearStorage();
                });
            }
            ...
            function DelRecordById(Id, LevelId) {
                $.tool.DataGet('CpComPatrols', 'ExaminePlanTemplateGetCheck', 'LevelId=' + LevelId,
                    ...
                    function callback(_Data) {
                        if (_Data == '1') {
                            $.tool.DataGet('CpComPatrols', 'DelPlanTemplateList_Patrol', 'Id=' + Id,
                                function Init() {
                                },
                                function callback(_Data) {
                                    LoadList();
            ...
            //审核计划模板
            function Examine(Id, LevelId, TaskTypeName) {
                $.tool.DataGet('CpComPatrols', 'ExaminePlanTemplateGetAuditing', 'LevelId=' + LevelId,
                    function Init() {
                    },
                    function callback(_Data) {
                        if (_Data == '1') {
                            if (TaskTypeName == '公区巡查') {
                                $.tool.DataPost('CpComPatrols', 'IsSetOtherPointPercentage', 'PlanId=' + Id,
                                    function Init() {
                                    },
                                    function callback(_Data) {
                                        if (_Data == '0') {
                                            HDialog.Open('500', '170', '../ComprehensivePatrols/PlanTemplateAuditing_Patrol.aspx?PlanId=' + Id, '审核', true, function callback(_Data) {
                                                LoadList();
                                            });
                                        } else {
                                            HDialog.Info("存在抽查点位未设置覆盖率，无法审核！");
                                        }
                                    },
                                    function completeCallback() {
                                    },
            ...
            function Enable(Id, LevelId) {
                $.tool.DataGet('CpComPatrols', 'ExaminePlanTemplateGetAuditing', 'LevelId=' + LevelId,
                    ...
                    function callback(_Data) {
                        if (_Data == '1') {
                            $.tool.DataGet('CpComPatrols', 'EnablePlanTemplateById_Patrol', 'PlanId=' + Id,
                                function Init() {
                                },
                                function callback(_Data) {
                                    HDialog.Info("操作成功！");
            ...
            function LoadList() {
                $("#TableContainerTemplate").datagrid({
                    url: '/HM/M_Main/HC/DataGetControl.aspx?Method=DataGet&Class=CpComPatrols&Command=GetPlanTemplateList_Patrol&' + $('#frmFrom').serialize(),
                    method: "get",
                    title: '',
                    loadMsg: '数据加载中,请稍候...',
                    nowrap: false,
