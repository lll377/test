<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="TaskStayExamine.aspx.cs" Inherits="M_Main.ComprehensivePatrols.TaskStayExamine" %>
    ...
    <form id="frmFrom" runat="server">
        ...
        <script type="text/javascript">
            ...
            var toolbar =
                [{
                    ...
                    handler: function () {
                        ...
                        if (row) {
                            //必须具有计划模板审核权限的岗位才能审核核查报告
                            $.tool.DataPostAsync('CpComPatrols', 'CheckIsAudit', 'TaskLevelId=' + row.TaskLevelId,
                                function Init() {
                                },
                                function callback(_Data) {
                                    if (_Data == "false") {
                                        HDialog.Info("必须具有计划模板审核权限的岗位才能审核核查报告");
                                    } else {
                                        HDialog.OpenWin('900', '560', "../ComprehensivePatrols/QualityVerificationReport.aspx?CommId=" + row.CommId + "&TaskId=" + row.TaskId + "&OpType=edit", '品质核查报告', false, function callback(_Data) {
                                            if (_Data != "") {//**获得返回的参数信息
                                                LoadList();
                                            }
                                        });
            ...
            var toolbarDetail =
                [{
                    ...
                    handler: function () {
                        ...
                        formData.append("TaskName", '品质任务点位');
                        $.ajax({
                            type: "POST",
            ...
            var columnDetail = [[
                ...
                { field: 'DidTime', title: '扫码时间', width: 100, align: 'left', sortable: true },
                {
                    field: 'xq', title: '任务对象/任务标准', width: 120, align: 'left', sortable: true, formatter: function (value, row, index) {
            ...
            function OnOtherPointPercentageList(TaskId) {
                HDialog.Open(600, 450, '../ComprehensivePatrols/TaskOtherPointPercentageList.aspx?TaskId=' + TaskId, '抽查点位覆盖率', true, function callback(_Data) {
                });
            }
            function OnFileList(TaskId, PointId) {
                HDialog.Open(600, 450, '../ComprehensivePatrols/ComprehensivePatrolsFiles.aspx?TaskId=' + TaskId + "&TaskPointId=" + TaskPointId, '查看附件', true, function callback(_Data) {
                });
            }

            function checkPatrolMsg(TaskId, TaskPointId) {
                HDialog.Open('900', '600', '../ComprehensivePatrols/CpPlanTaskStandard.aspx?TaskPointId=' + TaskPointId + "&TaskId=" + TaskId + "&CommId=" + $("#CommId").val(),
            ...
            function checkPlanTrajectory(TaskId, PlanId, CommId) {
                HDialog.Open('1100', '600', '../ComprehensivePatrols/TaskRouteFrame.aspx?TaskId=' + TaskId + '&PlanId=' + PlanId + '&CommId=' + CommId,
            ...
            function ShowMap(longitude) {
                HDialog.Open('700', '540', '../ComprehensivePatrols/PointShowMap.aspx?longitude=' + longitude, '查看地图', true, function callback(_Data) {
                });
            }
            //查看报事
            function TaskPointIncident(TaskPointId, TaskId) {
                HDialog.Open(800, 600, '../ComprehensivePatrols/TaskPointIncident.aspx?TaskPointId=' + TaskPointId + "&TaskId=" + TaskId + "&CommId=" + $("#CommId").val(), '查看报事', true, function callback(_Data) {
                });
            }
            ...
            function LoadList() {
                ...
                $("#DivContainer").datagrid({
                    url: '/HM/M_Main/HC/DataGetControl.aspx?Method=DataGet&Class=CpComPatrols&Command=GetCpPlanTaskList&Flag=待审核&' + $("#frmFrom").serialize(),
                    method: "get",
                    loadMsg: '数据加载中,请稍候...',
                    nowrap: false,
                    pageSize: 20,
            ...
            function LoadListDetail(Id) {
                $("#DivContainerDetail").datagrid({
                    ...
                    onBeforeLoad: function (param) {
                        ...
                        param.Command = "GetCpPlanTaskPointList";
                        param.TaskId = Id;
