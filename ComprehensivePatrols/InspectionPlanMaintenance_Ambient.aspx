<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="InspectionPlanMaintenance_Ambient.aspx.cs" Inherits="M_Main.ComprehensivePatrols.InspectionPlanMaintenance_Ambient" %>
        ...
        <script type="text/javascript">
            ...
            var toolbar = [{
                ...
                handler: function () {
                    if ($("#CommId").val().length < 3) {
                        ...
                        HDialog.Open('1100', '510', '../ComprehensivePatrols/TemporaryPatrolPlanGeneration_Ambient.aspx', '任务维护', true, function callback(_Data) {
            ...
            }, '-',
            {
                ...
                handler: function () {
                    ...
                    window.open("InspectionPlanMaintenanceExcel_Ambient.aspx?random=" + Math.random() + $('#frmFrom').serialize());
                }
            }, '-',
            {
                text: '筛选',
            ...
            function Edit(id, TaskLevelId) {
                ...
                $.tool.DataGet('CpComPatrols', 'ExaminePlanTemplateGetCheck', 'LevelId=' + TaskLevelId,
                    ...
                    function callback(_Data) {
                        if (_Data == '1') {
                            HDialog.Open('720', '250', '../ComprehensivePatrols/InspectionPlanMaintenanceEdit_Ambient.aspx?OpType=Edit&TaskId=' + id + '&CommId=' + $("#CommId").val(), '修改', true, function callback(_Data) {
                                LoadList();
                            });
                        }
                        else {
                    ...
                    function callback(_Data) {
                        if (_Data == '1') {
                            HDialog.Prompt('是否确定删除!', function () {
                                $.tool.DataGet('CpComPatrols', 'DelInspectionPlanMaintenance_Ambient', 'TaskId=' + TaskId + '&CommId=' + $("#CommId").val(),
                                    function Init() {
                                    },
                                    function callback(_Data) {
                                        LoadList();
            ...
            function Close(Id, TaskLevelId) {
                $.tool.DataGet('CpComPatrols', 'ExaminePlanTemplateGetAuditingBatch', 'LevelId=' + TaskLevelId,
                    ...
                    function callback(_Data) {
                        if (_Data == '1') {
                            HDialog.Open('720', '250', '../ComprehensivePatrols/InspectionPlanMaintenanceClose_Ambient.aspx?TaskId=' + Id + '&CommId=' + $("#CommId").val(), '关闭', true, function callback(_Data) {
                                LoadList();
                            });
                        }
                        else {
            ...
            function LoadList() {
                $("#TableContainer").datagrid({
                    url: '/HM/M_Main/HC/DataGetControl.aspx?Method=DataGet&Class=CpComPatrols&Command=GetInspectionPlanMaintenance_Ambient&' + $('#frmFrom').serialize(),
                    method: "get",
                    title: '',
                    loadMsg: '数据加载中,请稍候...',
                    nowrap: false,
