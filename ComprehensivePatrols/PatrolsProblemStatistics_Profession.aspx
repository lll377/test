<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="PatrolsProblemStatistics_Profession.aspx.cs" Inherits="M_Main.ComprehensivePatrols.PatrolsProblemStatistics_Profession" %>
...
<body>
    <form id="frmForm" runat="server">
        ...
        <input type="hidden" id="TaskName" name="TaskName" value="巡查问题统计-按所属专业" />
        <script type="text/javascript">
            ...
            var column = [
                [
                    { field: 'CommNum', rowspan: 2, title: '项目数量', width: 120, align: 'left', sortable: true },
                    { field: 'ProfessionName', rowspan: 2, title: '所属专业', width: 150, align: 'left', sortable: true },
                    { title: '按不合格标准统计', colspan: 3 },
                    { title: '按不合格扣分统计', colspan: 5 },
                    { field: 'TotalScore', rowspan: 2, title: '合计得分', width: 120, align: 'left', sortable: true }
                ],
                [
                    { field: 'TaskStandardNum', title: '核查标准数', width: 120, align: 'left', sortable: true },
                    { field: 'UnqualifiedTaskStandardNum', title: '不合格标准数', width: 120, align: 'left', sortable: true },
                    { field: 'UnqualifiedRate', title: '不合格标准占比（%）', width: 120, align: 'left', sortable: true },
                    { field: 'TaskStandardScore', title: '核查标准分', width: 120, align: 'left', sortable: true },
                    { field: 'UnqualifiedTaskStandardScore', title: '不合格扣分', width: 120, align: 'left', sortable: true },
                    { field: 'RiskTaskStandardScore', title: '风险性扣分', width: 120, align: 'left', sortable: true },
                    { field: 'ScoreSum', title: '合计扣分', width: 120, align: 'left', sortable: true },
                    { field: 'ScoreRate', title: '扣分占比（%）', width: 120, align: 'left', sortable: true }
                ]];
            ...
            var toolbar = [
                {
                    ...
                    handler: function () {
                        ...
                        //window.open("CpReportExcel.aspx?CommId=" + $('#CommId').val() + '&TypeId=' + $("#TypeId").val() + '&LevelId=' + $("#LevelId").combobox('getValues') + '&BeginTime=' + $('#BeginTime').val() + '&EndTime=' + $('#EndTime').val() + '&TaskName=巡查问题统计-按所属专业(得分)');
                        var formData = new FormData();
                        formData.append("CommId", $('#CommId').val());
                        formData.append("TypeId", $('#TypeId').val());
                        formData.append("LevelId", $("#LevelId").combobox('getValues'));
                        formData.append("BeginTime", $('#BeginTime').val());
                        formData.append("EndTime", $('#EndTime').val());
                        formData.append("TaskName", '巡查问题统计-按所属专业(得分)');
                        $.ajax({
                            type: "POST",
                            url: "/HM/M_Main/ComprehensivePatrols/CpReportExcel.aspx",
                            data: formData,
            ...
            function LoadList(str) {
                var urls = null;
                if (str != "") {
                    urls = '/HM/M_Main/HC/DataGetControl.aspx?Method=DataGet&Class=CpComPatrols&Command=PatrolsProblemStatistics_Profession&' + $("#frmForm").serialize();
                ...
                $("#TableContainer").datagrid({
                    ...
                    onBeforeLoad: function (param) {
                        if (str != "") {
                            param = $.JQForm.GetParam("CpComPatrols", "PatrolsProblemStatistics_Profession", "TableContainer", param);
                        }
                    }
                });
