<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="BudgetPlan_ProjectSummaryReport.aspx.cs" Inherits="M_Main.BudgetNew.Reports.BudgetPlan_ProjectSummaryReport" %>
...
<body style="margin: 0px; margin-top: 0px; overflow: hidden;">
    <form id="frmForm" runat="server">
        ...
        <script type="text/javascript">
            ...
            var column = [[
                { field: 'text', title: '单位名称', width: 220, align: 'left' },
                { field: 'TotalIncome', title: '收入合计', width: 100, align: 'left' },
                { field: 'TotalCost', title: '成本费用合计', width: 100, align: 'left' },
                { field: 'PreTaxProfit', title: '税前利润', width: 100, align: 'left' },
                { field: 'ProfitRateStr', title: '利润率', width: 100, align: 'left' },
                { field: 'ManagementCommission', title: '管理佣金', width: 100, align: 'left' }
            ]];
            ...
            var toolbar = [
                ...
                }, '-',
                {
                    ...
                    handler: function () {
                        ...
                        if (rowData.length == 0) {
                            HDialog.Info('没有需要导出的预算项目汇总数据');
                        }
                        else {
                            Export('预算项目汇总表', $('#TableContainer'));
                        }
                    }
                }
            ];
            ...
            function Btn_SearchClick() {
                ...
                $.tool.DataPost('Budget', 'Budget_BudgetPlan_ProjectSummaryReport', parameters,
                    function Init() {
                    },
            ...
            function ChangeToTable4(printDatagrid, HeadcolumnsList, columnsList) {
                ...
                var frozenColumn = undefined;  // 得到frozenColumns对象
                ...
                for (var i = 0; i < rows.length; i++) {
                    ...
                    for (var j = 0; j < nameList[0].length; j++) {
                        tableString += '\n<td';
                        if (nameList[0][j].SubjectName != 'undefined' && nameList[0][j].SubjectName != '') {
                            tableString += ' style="text-align:' + nameList[0][j].SubjectName + ';"';
                        }
                        tableString += ' >';
                        var FiledName = nameList[0][j].field;
                        if (FiledName == "text") {
                            tableString += Nbspstr + rows[i][FiledName];
                        } else {
