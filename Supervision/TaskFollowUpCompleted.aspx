<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="TaskFollowUpCompleted.aspx.cs" Inherits="M_Main.Supervision.TaskFollowUpCompleted" %>

<!DOCTYPE html>
<html xmlns="http://www.w3.org/1999/xhtml">
<head runat="server">
    <meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
    <title>任务跟进-已完成任务</title>
...
<body style="margin: 0px; padding: 0px; overflow: hidden">
    <form id="frmForm" runat="server">
        ...
        <input type="hidden" id="ExamineState" name="ExamineState" value="0" />
        ...
        <script type="text/javascript">
            ...
            var column = [[
                ...
                { field: 'IsSubmitExamineName', title: '是否生成报告', width: 200, align: 'left', sortable: true },
                {
                    field: 'ExamineState', title: '审核流程', width: 100, align: 'left', sortable: true, formatter: function (value, row, index) {
                        var str = "<a class=\"HrefStyle\" href=\"#\" onclick=\"ExamineInfo('" + row.TaskId + "')\">查看</a>";
                        return str;
                    }
                }
            ]];
            ...
            var toolbar = [
                ...
                , '-',
                {
                    ...
                    handler: function () {
                        ...
                        if (row == null) {
                            HDialog.Info("请选择需要生成报告的数据");
                            return;
                        }
                        var OpType = 'insert';
                        if (row.IsSubmitExamine == '1')
                            OpType = 'edit';
                        localStorage.setItem("OpType", OpType);
                        localStorage.setItem("TaskId", row.TaskId);
                        localStorage.setItem("CommId", $("#CommId").val());
                        localStorage.setItem("PlanId", row.PlanId);
                        HDialog.Open('800', '600', '../Supervision/TaskRectificationFrame.aspx', '生成整改报告', true, function callback(_Data) {
                ...
                , '-',
                {
                    ...
                    handler: function () {
                        ...
                        localStorage.setItem("OpType", 'see');
                        localStorage.setItem("TaskId", row.TaskId);
                        localStorage.setItem("CommId", $("#CommId").val());
                        localStorage.setItem("PlanId", row.PlanId);
                        HDialog.Open('800', '600', '../Supervision/TaskRectificationFrame.aspx', '预览整改报告', true, function callback(_Data) {
                            localStorage.removeItem('OpType');
                            localStorage.removeItem('TaskId');
                            localStorage.removeItem('CommId');
                            localStorage.removeItem('PlanId');
                ...
                , '-',
                {
                    ...
                    handler: function () {
                        ...
                        var urlParam = { "CommID": row.CommId, "TaskID": row.TaskId };
                        location.href = '/HM/M_Main/Supervision/GenerateReport.aspx?' + $.param(urlParam);
                    }
                }
                , '-',
                {
                    ...
                    handler: function () {
                        var row = $("#TableContainer").datagrid("getSelected");
                        if (row == null) {
                            HDialog.Info("请选择需要下载附件的数据");
                            return;
                        }
                        window.open("TaskFilesDownLoad.aspx?TaskId=" + row.TaskId + "&CommId=" + row.CommId);
                    }
                }
                , '-',
                {
                    ...
                    handler: function () {
                        ...
                        if (row.IsSubmitExamine == '0') {
                            HDialog.Info("请先生成报告");
                            return;
                        }
                        localStorage.setItem("OpType", 'submit');
                        localStorage.setItem("TaskId", row.TaskId);
                        localStorage.setItem("CommId", $("#CommId").val());
                        localStorage.setItem("PlanId", row.PlanId);
                        HDialog.Open('800', '600', '../Supervision/TaskRectificationFrame.aspx', '送审整改报告', true, function callback(_Data) {
                            localStorage.removeItem('OpType');
                            localStorage.removeItem('TaskId');
                            localStorage.removeItem('CommId');
                            localStorage.removeItem('PlanId');
            ...
            function StandardList(TaskId) {
                HDialog.Open('1000', '700', '../Supervision/TaskStandardList.aspx?NeedEdit=1&TaskId=' + TaskId + '&CommId=' + $("#CommId").val(), '任务标准列表', true, function callback(_Data) {
                    LoadList();
                });
            }

            function ExamineInfo(TaskId) {
                HDialog.Open('800', '400', '../Supervision/TaskRectificationInfoExamineInfo.aspx?TaskId=' + TaskId + '&CommId=' + $("#CommId").val(), '查看审核流程', true, function callback(_Data) {
            ...
            function Export() {
                ...
                $.tool.DataPostChk('frmForm', 'CSSupervision', 'TaskList', $('#frmForm').serialize() + '&page=1&rows=999999999',
                    ...
                    function callback(_Data) {
                        ...
                        i.appendTo(f);
                        var date = new Date();
                        l.val('检查任务跟进-已完成任务-' + dateFormat("YYYY-mm-dd", date));
                        l.appendTo(f);
