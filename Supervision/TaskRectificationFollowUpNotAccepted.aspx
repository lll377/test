<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="TaskRectificationFollowUpNotAccepted.aspx.cs" Inherits="M_Main.Supervision.TaskRectificationFollowUpNotAccepted" %>

<!DOCTYPE html>
<html xmlns="http://www.w3.org/1999/xhtml">
<head runat="server">
    <meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
    <title>问题整改跟进-未验收任务</title>
...
<body style="margin: 0px; padding: 0px; overflow: hidden">
    <form id="frmForm" runat="server">
        ...
        <input type="hidden" id="TaskCheckTypeId" name="TaskCheckTypeId" value="" />
        ...
        <input type="hidden" id="IsAccepted" name="IsAccepted" value="0" />
        ...
        <script type="text/javascript">
            ...
            function LoadList() {
                $("#TableContainer").datagrid({
                    ...
                    onClickRow: function (rowIndex, rowData) {
                        ...
                        $("#TaskCheckTypeId").val(rowData.CheckTypeId);
            ...
            var toolbarDeductionItem = [
                {
                    ...
                    iconCls: 'icon-edit',
                    handler: function () {
                        var row = $("#TableContainerDeductionItem").datagrid("getSelected");
                        if (row == null) {
                            HDialog.Info("请选择修改数据");
                            return;
                        }
                        if (row.IsNeedRectification == '0') {
                            HDialog.Info("选中数据无需整改");
                            return;
                        }
                        Rectification(row.TaskRectificationDeductionItemId);
                    }
                }
                , '-',
                {
                    ...
                    handler: function () {
                        ...
                        if (row.IsRectification != '1') {
                            HDialog.Info("选中数据未完成整改，无法验收");
                            return;
                        }
                        Accepted(row.TaskRectificationDeductionItemId);
                    }
                }
                , '-',
                {
            ...
            function Rectification(TaskRectificationDeductionItemId) {
                $.tool.DataGet('CSSupervision', 'CanReportWriting', 'Id=' + $("#TaskCheckTypeId").val(),
                    function Init() {
                    },
                    function callback(_Data) {
                        ...
                        HDialog.Open('800', '600', '../Supervision/TaskRectificationDeductionEdit.aspx?OpType=Rectification&CommId=' + $("#CommId").val() + '&TaskId=' + $("#TaskId").val() + '&TaskRectificationDeductionItemId=' + TaskRectificationDeductionItemId, '完成整改', true, function callback(_Data) {
                            LoadListDeductionItem();
                        });
                    },
            ...
            function Accepted(TaskRectificationDeductionItemId) {
                $.tool.DataGet('CSSupervision', 'CanRectificationOfProblems', 'Id=' + $("#TaskCheckTypeId").val(),
                    function Init() {
                    },
                    function callback(_Data) {
                        if (_Data == "0") {
                        ...
                        HDialog.Open('800', '600', '../Supervision/TaskRectificationDeductionEdit.aspx?OpType=Accepted&CommId=' + $("#CommId").val() + '&TaskId=' + $("#TaskId").val() + '&TaskRectificationDeductionItemId=' + TaskRectificationDeductionItemId, '完成验收', true, function callback(_Data) {
                            //LoadList();
                            //$("#TaskId").val('');
                            //$("#TaskCheckTypeId").val('');
                            LoadListDeductionItem();
            ...
            function Export() {
                ...
                $.tool.DataPostChk('frmForm', 'CSSupervision', 'TaskList', $('#frmForm').serialize() + '&page=1&rows=999999999',
                    ...
                    function callback(_Data) {
                        ...
                        i.appendTo(f);
                        var date = new Date();
                        l.val('检问题整改跟进-未验收任务-' + dateFormat("YYYY-mm-dd", date));
                        l.appendTo(f);
            ...
            function ExportDeductionItem() {
                ...
                $.tool.DataPostChk('frmForm', 'CSSupervision', 'TaskRectificationDeductionItemList', $('#frmForm').serialize() + '&page=1&rows=999999999',
                    ...
                    function callback(_Data) {
                        ...
                        i.appendTo(f);
                        var date = new Date();
                        l.val('检问题整改跟进-未验收任务-加扣分项-' + dateFormat("YYYY-mm-dd", date));
                        l.appendTo(f);
                        f.appendTo(document.body).submit();
                        $('#SearchDlg').dialog('close');
                    },
