<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="List.aspx.cs" Inherits="M_Main.SysNew.EmpowerProject.List" %>
...
<body>
    ...
    <div id="dlg_search" class="easyui-dialog" title="筛选岗位" style="width: 1024px; height: 423px; max-width: 1024px; padding: 0px" data-options="
    ...
    <script type="text/javascript">
        ...
        function Save() {
            ...
            $.dataPostJson('Empower_Project', 'Save', { "ListRoleData": value.substr(1) }, true, false,
                function Init() {
                },
                function callback(data) {
                    if (data.result) {
        ...
        var frozenColumns = [[
            { field: 'text', title: '项目名称', width: 500, align: 'left', halign: 'center', sortable: false }
        ]];
        var column = [[{
                ...
                $.each(roleValue.split('|'), function (i, value) {
                    ...
                    column[0].push({
                        ...
                        formatter: function (value, row, index) {
                            ...
                            //var organCode = row.id;
                            ////区域编号
                            //if (isValueNull(row.attributes.CommID) && row.attributes.CommID != 0) {
                            //    organCode = row.attributes.PID;
                            //}
                            return "<input " + checked + " type=\"checkbox\" submitvalue='" + roleValue + "," + row.attributes.OrganCode + "," + row.attributes.CommID + "," + row.attributes.TypeName + "'  id='cbCol_" + roleValue + "_cbRow_" + row.id + "' onclick=\"SelTgCheckBox(this);\" value=" + checkValue + " />";
                        }
                    });
                });
            }
        ...
        function InitTreeGrid() {
            ...
            $("#tg").treegrid({
                ...
                onBeforeLoad: function (row, param) {
                    param.Class = "Empower_Project";
                    param.Method = "GetList";
                    param.r = Math.random();
                    param.hdRoleValue = $("#hdRoleValue").val();
                    //param = $.getDataGridParam("Empower_Project", "GetList", param);
                },
