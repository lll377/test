<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="rsda.aspx.cs" Inherits="M_Main.PersonnelManage.rsda" %>
...
<body>
    <form id="frmForm" runat="server" clientidmode="Static">
        ...
        <!-- 部门树class="easyui-resizable"  data-options="minWidth:50,minHeight:50" -->
        <div>
            <div id="LeftPanel">
                <ul class="ztree" id="treeDemo">
                </ul>
            </div>
            <%--   <div id="arrowborder" style="float: left; width: 5px; background-color: Blue; cursor: w-resize;">
            </div>--%>
            <!-- 人员列表 -->
            <%-- <div id="rightbar" style="width: 2px; background: #cccccc; cursor: e-resize; float: left; height: auto;">--%>
            ...
            <table style="width: 100%; font-size: 12px; font-family: 微软雅黑;">
                ...
                <tr>
                    <td colspan="8" align="center">
                        <a href="#" class="easyui-linkbutton" data-options="iconCls:'icon-search'" onclick="LoadList('','',1,20);">确定筛选</a>
                    </td>
                </tr>
            </table>
        </div>
        ...
        <input type="hidden" id="HiPageIndex" name="HiPageIndex" value="1" runat="server" />
        <input type="hidden" id="HiPageSize" name="HiPageSize" value="20" runat="server" />
        ...
        <input type="hidden" id="PersonType1" name="PersonType1" value="" />
        <input type="hidden" id="BlacklistPerson" name="BlacklistPerson" runat="server" value="0" />

        <input type="hidden" id="hiLoginCorpCode" name="hiLoginCorpCode" runat="server"/>
        ...
        <script language="javascript" type="text/javascript">
            ...
            //        var _lw = $("#LeftPanel").width();
            //        var _rw = $("#RightPanel").width();
            //        if ((_lw + _w > minwidth && _w < 0) || (_lw + _w < maxwidth && _w > 0)) {
            //            $("#LeftPanel").width(_lw + _w);
            //            $("#RightPanel").width(_rw - _w);
            //        }
            //        else {
            //            if (_w > 0) {
            //                $("#LeftPanel").width(maxwidth);
            //                $("#RightPanel").width(_rw - (maxwidth - _lw));
            //            }
            //            else {
            //                $("#LeftPanel").width(minwidth);
            //                $("#RightPanel").width(_rw + (_lw - minwidth));
            //            }
            //        }
            //    }
            //});
            ...
            //    //初始化参数
            //    var els = document.getElementById('LeftPanel').style;
            //    //鼠标的 X 和 Y 轴坐标
            //    x = 0;
            //    //邪恶的食指
            //    $(el).mousedown(function (e) {
            //        //按下元素后，计算当前鼠标与对象计算后的坐标
            //        x = e.clientX - el.offsetWidth - $("#LeftPanel").width();
            //        //在支持 setCapture 做些东东
            //        el.setCapture ? (
            //            //捕捉焦点
            //            el.setCapture(),
            ...
            //    $("#LeftPanel").height(totalHeight - topHeight);
            ...
            function GetParam(sort, order, Pn, Ps) {
                ...
                if (GetQueryString('flag') != undefined && GetQueryString("flag") != null
                    && GetQueryString('flag') != 'null' && GetQueryString("flag") != "") {
                    str += '&flag=' + GetQueryString('flag');
                }
                ...
                if (GetQueryString('PersonType') != undefined && GetQueryString('PersonType') != null
                    && GetQueryString('PersonType') != 'null' && GetQueryString("PersonType") != "") {
                    str += '&PersonType1=' + GetQueryString('PersonType');
                }
                if ($("#SortDepCode").val() != undefined && $("#SortDepCode").val() != null
                    && $("#SortDepCode").val() != 'null' && $("#SortDepCode").val() != "") {
                    str += '&SortDepCode=' + $("#SortDepCode").val();
                }
                ...
                var DimissionReasonArr = $("input:hidden[name='DimissionReason']");
                var DimissionReason = '';
                $.each(DimissionReasonArr, function (index, item) {
                    DimissionReason += ',' + item.value;
                });
                if (DimissionReason.length > 0);
                {
                    str += '&DimissionReasonName=' + DimissionReason.substr(1, DimissionReason.length - 1)
                }
                ...
                $("#HiSort").val(sort);
                $("#HiOrder").val(order);
                str += '&HiSort=' + sort + '&HiOrder=' + order + '&page=' + Pn + '&rows=' + Ps;
                //2017-06-14 添加 是否黑名单
                str += '&BlacklistPerson=' + $('#BlacklistPerson').val();
            ...
            var frozenColumns = [[
                {
                    field: 'Name', title: '姓名', width: 100, align: 'left', sortable: true, formatter: function (value, row, index) {
                        if ($("#hiLoginCorpCode").val() == "1824") {
                            var str = "<a style=\"color:blue;\" href=\"rada_xx_1824.aspx?OpType=edit&show=1&Id=" + row.Id + "&HiPageIndex=" + $("#HiPageIndex").val() + "&HiPageSize=" + $("#HiPageSize").val() + "\">" + row.Name + "</a>";
                            return str;
                        } else {
                            var str = "<a style=\"color:blue;\" href=\"rsda_xx.aspx?OpType=edit&show=1&Id=" + row.Id + "&HiPageIndex=" + $("#HiPageIndex").val() + "&HiPageSize=" + $("#HiPageSize").val() + "\">" + row.Name + "</a>";
                        return str;
                        }
            ...
            var column = [[
                ...
                //    }
                //},
                { field: 'parantDepName', title: '人事所属机构', width: 300, align: 'left', sortable: true },
            ...
            var toolbar = [
                    ...
                    handler: function () {
                        if ($("#hiLoginCorpCode").val() == "1824") {
                            CheckAuthority('1001', function callback(_Data) {
                                ...
                                if (_Data == "true") {
                                    window.location.href = 'rada_xx_1824.aspx?OpType=insert';
                                }
                                else {
                                    $.messager.alert('错误', '无此权限', 'error');
                                }
                            ...
                            CheckAuthority('1001', function callback(_Data) {
                                ...
                                if (_Data == "true") {
                                    window.location.href = 'rsda_xx.aspx?OpType=insert';
                                }
                                else {
                                    $.messager.alert('错误', '无此权限', 'error');
                                }
                ...
                }, '-',
                {
                    ...
                    handler: function () {
                        var h = $(window).height();
                        HDialog.Open("960", h, "../PersonnelManage/PersonlImportFrame.aspx", '批量导入', false, function callback(_Data) {
                            if (_Data == "true") {
                                LoadList("", "", 1, 20);
                            }
                        });
                ...
                }, '-',
                {
                    ...
                    handler: function () {
                        ...
                        if (row == "" || row == null || row == undefined) {
                            $.messager.alert('提示', '请选择查看数据', '');
                            return;
                        }
                        window.location.href = "PersonnelFiles.aspx?Id=" + row.Id;
                    }
                }, '-',
                {
                    text: '查看人员考勤',
                    ...
                    handler: function () {
                        ...
                        if (row.WorkSign == "" || row.WorkSign == null || row.WorkSign == undefined) {
                            $.messager.alert('提示', '该人员未生成员工工号', '');
                            return;
                        } else {
                            //判断该员工是否存在考勤记录
                            $.ajax({
                                url: '/HM/M_Main/HC/DataGetControl.aspx?Method=DataGet&Class=CSPerson&Command=GetPersonCheckWorkDetailed&WorkSign=' + row.WorkSign,
                                type: 'GET',
                                dataType: 'text',
                                cache: false,
                                success: function (_Data) {
                                    if (_Data == "true") {
                                        HDialog.Open('900', '640', "../PersonnelManage/ckrykq.aspx?WorkSign=" + row.WorkSign + "&Ram=" + Math.random(), '查看人员考勤信息', false, function callback(_Data) {

                                        });
                                    } else {
                                        $.messager.alert('提示', '未找到该人员的考勤信息', '');
                                    }
                                },
                ...
                }, '-',
                {
                    ...
                    iconCls: 'icon-edit',
                    handler: function () {
                        if ($('#BlacklistPerson').val() == "0") { 
                            $('#BlacklistPerson').val("1");
                            LoadList('', '', 1, 20);
                        }
                        else 
                        {
                            $('#BlacklistPerson').val("0");
                            LoadList('', '', 1, 20);
                        }
                    }
                }, '-',
                {
                    ...
                    handler: function () {
                        $('#BlacklistPerson').val("1");
                        window.open("BlacklistPersonExport.aspx?random=" + Math.random() + GetParam());
                    }
                }
            ];
            ...
            $("#TableContainer").datagrid({
                onSortColumn: function (sort, order) {
                    LoadList(sort, order, 1, 20);
                }
            })

            //function LoadList(sort, order, Pn) {
            //    $("#TableContainer").datagrid({
            //        url: '/HM/M_Main/HC/DataGetControl.aspx?Method=DataGet&Class=CSPerson&Command=SelectPersonList' + GetParam(sort, order, Pn),
            //        method: "get",
            //        title: '人事档案列表',
            //        nowrap: false,
            //        pageSize: 20,
            ...
            function LoadList(sort, order, page, rows) {
                //debugger
                var filter = GetQueryString("filter");
                if(filter && filter!=null && filter!="" && filter!="undefined"){
                    if(filter=="CurMonthEntry"){
                        $("#StartEntryDate").val(getCurrentMonthFirst());
                        $("#EndEntryDate").val(getCurrentMonthLast());
                    }else{
                        $("#WorkDimissionDateStart").val(getCurrentMonthFirst());
                        $("#WorkDimissionDateEnd").val(getCurrentMonthLast());
                        $("#Chk1").removeAttr("checked");
                        $("#Chk2").removeAttr("checked");
                        $("#Chk3").removeAttr("checked");
                        $("#Chk4").removeAttr("checked");
                        $("#Chk5").attr("checked","checked");
                    }
                }
               
                $.tool.DataPost2('CSPerson', 'SelectPersonList', GetParam(sort, order, page, rows),
                    function Init() {
                    },
                    function callback(_Data) {
            ...
            if ($("#HiPageIndex").val() != "" && $("#HiPageIndex").val() != null && $("#HiPageIndex").val() != undefined &&
                $("#HiPageSize").val() != "" && $("#HiPageSize").val() != null && $("#HiPageSize").val() != undefined) {

                if ($('#TableContainer').datagrid('options') != null) {
                    $('#TableContainer').datagrid('options').pageNumber = parseInt($("#HiPageIndex").val());
                    $('#TableContainer').datagrid('options').pageSize = parseInt($("#HiPageSize").val());
                }
              
                //alert($('#TableContainer').datagrid('options').pageNumber + "--" + $('#TableContainer').datagrid('options').pageSize);
                LoadList("", "", $("#HiPageIndex").val(), $("#HiPageSize").val());
            }
            ...
            function Save() {
                ...
                document.getElementById("HiIFCode").value = "";
