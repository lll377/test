<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="NewCustomerBankLiveLockBrowse.aspx.cs" Inherits="M_Main.ResourceBaseAuditing.NewCustomerBankLiveLockBrowse" %>
...
<script type="text/javascript">
    ...
    var column = [[
        ...
        { field: 'CommName', title: '小区名称', width: 100, align: 'left', sortable: true },
        { field: 'RoomSign', title: '房屋编号', width: 100, align: 'left', sortable: true },
        { field: 'BuildArea', title: '建筑面积', width: 110, align: 'left', sortable: true },
        { field: 'StateName', title: '交房状态', width: 110, align: 'left', sortable: true },
        { field: 'RoomName', title: '房屋路址', width: 100, align: 'left', sortable: true },
        ...
        {
            field: 'IsLockBank', title: '是否锁定', width: 100, align: 'left', sortable: true, formatter: function (value, row, index) {
                var str = "否";
                if (value == 1) {
                    str = "是";
                }
            ...
            handler: function () {
                ...
                if (rows == "") {
                    ...
                    $.each(rows, function (key, val) {
                        LiveIDs += rows[key]["LiveID"] + ",";
                    });
                    LiveIDs = LiveIDs.substr(0, LiveIDs.length - 1);
                    Lock(LiveIDs, 1);
                }
            }
        }, '-', {
    ...
    function LoadList() {
        $("#TableContainer").datagrid({
            ...
            onBeforeLoad: function (param) {
                param = $.JQForm.GetParam("CustBank", "CustomerLiveBankList", "TableContainer", param);
            }
        });
        $("#SearchDlg").dialog("close");
    }

    function Lock(LiveIDs, IsLock) {
        var LockType = "锁定";
        if (IsLock == 0) {
            LockType = "解锁";
        }
        ...
        if (isCheck) {
            $.messager.confirm('确定', LockType, function (r) {
                if (r) {
                    $.tool.DataPostChk('frmForm', 'ResourceBaseAuditing', 'NewCustomerBankLiveLockBrowse', '&KeyIDValues=' + LiveIDs + '&IsLock=' + IsLock,
                        function Init() {
                        },
